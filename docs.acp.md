...
This document describes how the Sage ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`sage acp` exposes an ACP agent over stdio and forwards prompts to a running
Sage Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Sage Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `sage acp` over stdio.

Example config:

```bash
sage config set gateway.remote.url wss://gateway-host:18789
sage config set gateway.remote.token <token>
```

Example run:

```bash
sage acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
sage acp --session agent:main:main
sage acp --session agent:design:main
sage acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Sage ACP” to start a thread.

## Execution Model

- ACP client spawns `sage acp` and speaks ACP messages over stdio.
...
```bash
sage acp --session agent:main:main
sage acp --session-label "support inbox"
sage acp --reset-session
```
...
`sage acp` resolves the Gateway URL and auth from CLI flags or config:
