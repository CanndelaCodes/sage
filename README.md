# Sage — Build with words, not code.

<p align="center">
  <a href="https://github.com/CanndelaCodes/sage"><strong>Sage</strong></a> — Your autonomous AI assistant
</p>

<p align="center">
  <a href="https://github.com/CanndelaCodes/sage/actions/workflows/ci.yml?branch=sage/main"><img src="https://img.shields.io/github/actions/workflow/status/CanndelaCodes/sage/ci.yml?branch=sage/main&style=for-the-badge" alt="CI status"></a>
  <a href="https://github.com/CanndelaCodes/sage/releases"><img src="https://img.shields.io/github/v/release/CanndelaCodes/sage?include_prereleases&style=for-the-badge" alt="GitHub release"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg?style=for-the-badge" alt="MIT License"></a>
</p>

**Sage** is an autonomous AI assistant that runs on your own devices. It is a vibecoder-first fork of [OpenClaw](https://github.com/openclaw/openclaw), redesigned to make AI-powered automation accessible to non-technical users.
...
- Vibecoder Dashboard as default UI

[Getting Started](docs/start/getting-started.md) · [Docs](docs/) · [License](LICENSE)

Preferred setup: run the onboarding wizard (`sage onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (native -- no WSL2 required)**.
Works with npm, pnpm, or bun.
New install? Start here: [Getting started](https://docs.sage.ai/start/getting-started)
...
Model note: while any model is supported, I strongly recommend **Anthropic Pro/Max (100/200) + Opus 4.5** for long‑context strength and better prompt‑injection resistance. See [Onboarding](https://docs.sage.ai/start/onboarding).

## Models (selection + auth)

- Models config + CLI: [Models](https://docs.sage.ai/concepts/models)
- Auth profile rotation (OAuth vs API keys) + fallbacks: [Model failover](https://docs.sage.ai/concepts/model-failover)
...
Full beginner guide (auth, pairing, channels): [Getting started](https://docs.sage.ai/start/getting-started)

```bash
sage onboard --install-daemon

sage gateway --port 18789 --verbose

# Send a message
sage message send --to +1234567890 --message "Hello from Sage"

# Talk to the assistant (optionally deliver back to any connected channel: WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
sage agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.sage.ai/install/updating) (and run `sage doctor`).
...
Switch channels (git + npm): `sage update --channel stable|beta|dev`.
Details: [Development channels](https://docs.sage.ai/install/development-channels).
...
```bash
git clone https://github.com/CanndelaCodes/sage.git
cd sage
...
pnpm sage onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm sage ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `sage` binary.

## Security defaults (DM access)

Sage connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.

Full security guide: [Security](https://docs.sage.ai/gateway/security)

Default behavior on Telegram/WhatsApp/Signal/iMessage/Microsoft Teams/Discord/Google Chat/Slack:

- **DM pairing** (`dmPolicy="pairing"` / `channels.discord.dm.policy="pairing"` / `channels.slack.dm.policy="pairing"`): unknown senders receive a short pairing code and the bot does not process their message.
- Approve with: `sage pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `sage doctor` to surface risky/misconfigured DM policies.

## Highlights

- **[Local-first Gateway](https://docs.sage.ai/gateway)** — single control plane for sessions, channels, tools, and events.
- **[Multi-channel inbox](https://docs.sage.ai/channels)** — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, BlueBubbles (iMessage), iMessage (legacy), Microsoft Teams, Matrix, Zalo, Zalo Personal, WebChat, macOS, iOS/Android.
- **[Multi-agent routing](https://docs.sage.ai/gateway/configuration)** — route inbound channels/accounts/peers to isolated agents (workspaces + per-agent sessions).
- **[Voice Wake](https://docs.sage.ai/nodes/voicewake) + [Talk Mode](https://docs.sage.ai/nodes/talk)** — always-on speech for macOS/iOS/Android with ElevenLabs.
- **[Live Canvas](https://docs.sage.ai/platforms/mac/canvas)** — agent-driven visual workspace with [A2UI](https://docs.sage.ai/platforms/mac/canvas#canvas-a2ui).
- **[First-class tools](https://docs.sage.ai/tools)** — browser, canvas, nodes, cron, sessions, and Discord/Slack actions.
- **[Companion apps](https://docs.sage.ai/platforms/macos)** — macOS menu bar app + iOS/Android [nodes](https://docs.sage.ai/nodes).
- **[Onboarding](https://docs.sage.ai/start/wizard) + [skills](https://docs.sage.ai/tools/skills)** — wizard-driven setup with bundled/managed/workspace skills.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=sage/sage&type=date&legend=top-left)](https://www.star-history.com/#sage/sage&type=date&legend=top-left)

## Everything we built so far

### Core platform

- [Gateway WS control plane](https://docs.sage.ai/gateway) with sessions, presence, config, cron, webhooks, [Control UI](https://docs.sage.ai/web), and [Canvas host](https://docs.sage.ai/platforms/mac/canvas#canvas-a2ui).
- [CLI surface](https://docs.sage.ai/tools/agent-send): gateway, agent, send, [wizard](https://docs.sage.ai/start/wizard), and [doctor](https://docs.sage.ai/gateway/doctor).
- [Pi agent runtime](https://docs.sage.ai/concepts/agent) in RPC mode with tool streaming and block streaming.
- [Session model](https://docs.sage.ai/concepts/session): `main` for direct chats, group isolation, activation modes, queue modes, reply-back. Group rules: [Groups](https://docs.sage.ai/concepts/groups).
- [Media pipeline](https://docs.sage.ai/nodes/images): images/audio/video, transcription hooks, size caps, temp file lifecycle. Audio details: [Audio](https://docs.sage.ai/nodes/audio).

### Channels

- [Channels](https://docs.sage.ai/channels): [WhatsApp](https://docs.sage.ai/channels/whatsapp) (Baileys), [Telegram](https://docs.sage.ai/channels/telegram) (grammY), [Slack](https://docs.sage.ai/channels/slack) (Bolt), [Discord](https://docs.sage.ai/channels/discord) (discord.js), [Google Chat](https://docs.sage.ai/channels/googlechat) (Chat API), [Signal](https://docs.sage.ai/channels/signal) (signal-cli), [BlueBubbles](https://docs.sage.ai/channels/bluebubbles) (iMessage, recommended), [iMessage](https://docs.sage.ai/channels/imessage) (legacy imsg), [Microsoft Teams](https://docs.sage.ai/channels/msteams) (extension), [Matrix](https://docs.sage.ai/channels/matrix) (extension), [Zalo](https://docs.sage.ai/channels/zalo) (extension), [Zalo Personal](https://docs.sage.ai/channels/zalouser) (extension), [WebChat](https://docs.sage.ai/web/webchat).
- [Group routing](https://docs.sage.ai/concepts/group-messages): mention gating, reply tags, per-channel chunking and routing. Channel rules: [Channels](https://docs.sage.ai/channels).

### Apps + nodes

- [macOS app](https://docs.sage.ai/platforms/macos): menu bar control plane, [Voice Wake](https://docs.sage.ai/nodes/voicewake)/PTT, [Talk Mode](https://docs.sage.ai/nodes/talk) overlay, [WebChat](https://docs.sage.ai/web/webchat), debug tools, [remote gateway](https://docs.sage.ai/gateway/remote) control.
- [iOS node](https://docs.sage.ai/platforms/ios): [Canvas](https://docs.sage.ai/platforms/mac/canvas), [Voice Wake](https://docs.sage.ai/nodes/voicewake), [Talk Mode](https://docs.sage.ai/nodes/talk), camera, screen recording, Bonjour pairing.
- [Android node](https://docs.sage.ai/platforms/android): [Canvas](https://docs.sage.ai/platforms/mac/canvas), [Talk Mode](https://docs.sage.ai/nodes/talk), camera, screen recording, optional SMS.
- [macOS node mode](https://docs.sage.ai/nodes): system.run/notify + canvas/camera exposure.

### Tools + automation

- [Browser control](https://docs.sage.ai/tools/browser): dedicated sage Chrome/Chromium, snapshots, actions, uploads, profiles.
- [Canvas](https://docs.sage.ai/platforms/mac/canvas): [A2UI](https://docs.sage.ai/platforms/mac/canvas#canvas-a2ui) push/reset, eval, snapshot.
- [Nodes](https://docs.sage.ai/nodes): camera snap/clip, screen record, [location.get](https://docs.sage.ai/nodes/location-command), notifications.
- [Cron + wakeups](https://docs.sage.ai/automation/cron-jobs); [webhooks](https://docs.sage.ai/automation/webhook); [Gmail Pub/Sub](https://docs.sage.ai/automation/gmail-pubsub).
- [Skills platform](https://docs.sage.ai/tools/skills): bundled, managed, and workspace skills with install gating + UI.

### Runtime + safety

- [Channel routing](https://docs.sage.ai/concepts/channel-routing), [retry policy](https://docs.sage.ai/concepts/retry), and [streaming/chunking](https://docs.sage.ai/concepts/streaming).
- [Presence](https://docs.sage.ai/concepts/presence), [typing indicators](https://docs.sage.ai/concepts/typing-indicators), and [usage tracking](https://docs.sage.ai/concepts/usage-tracking).
- [Models](https://docs.sage.ai/concepts/models), [model failover](https://docs.sage.ai/concepts/model-failover), and [session pruning](https://docs.sage.ai/concepts/session-pruning).
- [Security](https://docs.sage.ai/gateway/security) and [troubleshooting](https://docs.sage.ai/channels/troubleshooting).

### Ops + packaging

- [Control UI](https://docs.sage.ai/web) + [WebChat](https://docs.sage.ai/web/webchat) served directly from the Gateway.
- [Tailscale Serve/Funnel](https://docs.sage.ai/gateway/tailscale) or [SSH tunnels](https://docs.sage.ai/gateway/remote) with token/password auth.
- [Nix mode](https://docs.sage.ai/install/nix) for declarative config; [Docker](https://docs.sage.ai/install/docker)-based installs.
- [Doctor](https://docs.sage.ai/gateway/doctor) migrations, [logging](https://docs.sage.ai/logging).
...
- **[Gateway WebSocket network](https://docs.sage.ai/concepts/architecture)** — single WS control plane for clients, tools, and events (plus ops: [Gateway runbook](https://docs.sage.ai/gateway)).
- **[Tailscale exposure](https://docs.sage.ai/gateway/tailscale)** — Serve/Funnel for the Gateway dashboard + WS (remote access: [Remote](https://docs.sage.ai/gateway/remote)).
- **[Browser control](https://docs.sage.ai/tools/browser)** — sage‑managed Chrome/Chromium with CDP control.
- **[Canvas + A2UI](https://docs.sage.ai/platforms/mac/canvas)** — agent‑driven visual workspace (A2UI host: [Canvas/A2UI](https://docs.sage.ai/platforms/mac/canvas#canvas-a2ui)).
- **[Voice Wake](https://docs.sage.ai/nodes/voicewake) + [Talk Mode](https://docs.sage.ai/nodes/talk)** — always‑on speech and continuous conversation.
- **[Nodes](https://docs.sage.ai/nodes)** — Canvas, camera snap/clip, screen record, `location.get`, notifications, plus macOS‑only `system.run`/`system.notify`.

## Tailscale access (Gateway dashboard)

Sage can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (Sage enforces this).
- Serve can be forced to require a password by setting `gateway.auth.mode: "password"` or `gateway.auth.allowTailscale: false`.
- Funnel refuses to start unless `gateway.auth.mode: "password"` is set.
- Optional: `gateway.tailscale.resetOnExit` to undo Serve/Funnel on shutdown.

Details: [Tailscale guide](https://docs.sage.ai/gateway/tailscale) · [Web surfaces](https://docs.sage.ai/web)
...
Details: [Remote access](https://docs.sage.ai/gateway/remote) · [Nodes](https://docs.sage.ai/nodes) · [Security](https://docs.sage.ai/gateway/security)
...
Details: [Nodes](https://docs.sage.ai/nodes) · [macOS app](https://docs.sage.ai/platforms/macos) · [Gateway protocol](https://docs.sage.ai/concepts/architecture)
...
Details: [Session tools](https://docs.sage.ai/concepts/session-tool)

## Skills registry (SageSkills)

SageSkills is a minimal skill registry. With SageSkills enabled, the agent can search for skills automatically and pull in new ones as needed.

[SageSkills](https://sageskills.com)
...
- Controlled via `sage nodes …`.

Runbook: [iOS connect](https://docs.sage.ai/platforms/ios).

### Android node (optional)

- Pairs via the same Bridge + pairing flow as iOS.
- Exposes Canvas, Camera, and Screen capture commands.
- Runbook: [Android connect](https://docs.sage.ai/platforms/android).

## Agent workspace + skills

- Workspace root: `~/.sage/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.sage/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.sage/sage.json` (model + defaults):
...
[Full configuration reference (all keys + examples).](https://docs.sage.ai/gateway/configuration)
...
Details: [Security guide](https://docs.sage.ai/gateway/security) · [Docker + sandboxing](https://docs.sage.ai/install/docker) · [Sandbox config](https://docs.sage.ai/gateway/configuration)

### [WhatsApp](https://docs.sage.ai/channels/whatsapp)

- Link the device: `pnpm sage channels login` (stores creds in `~/.sage/credentials`).
- Allowlist who can talk to the assistant via `channels.whatsapp.allowFrom`.
- If `channels.whatsapp.groups` is set, it becomes a group allowlist; include `"*"` to allow all.

### [Telegram](https://docs.sage.ai/channels/telegram)
...
### [Slack](https://docs.sage.ai/channels/slack)

- Set `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` (or `channels.slack.botToken` + `channels.slack.appToken`).

### [Discord](https://docs.sage.ai/channels/discord)
...
### [Signal](https://docs.sage.ai/channels/signal)

- Requires `signal-cli` and a `channels.signal` config section.

### [BlueBubbles (iMessage)](https://docs.sage.ai/channels/bluebubbles)

- **Recommended** iMessage integration.
- Configure `channels.bluebubbles.serverUrl` + `channels.bluebubbles.password` and a webhook (`channels.bluebubbles.webhookPath`).
- The BlueBubbles server runs on macOS; the Gateway can run on macOS or elsewhere.

### [iMessage (legacy)](https://docs.sage.ai/channels/imessage)

- Legacy macOS-only integration via `imsg` (Messages must be signed in).
- If `channels.imessage.groups` is set, it becomes a group allowlist; include `"*"` to allow all.

### [Microsoft Teams](https://docs.sage.ai/channels/msteams)

- Configure a Teams app + Bot Framework, then add a `msteams` config section.
- Allowlist who can talk via `msteams.allowFrom`; group access via `msteams.groupAllowFrom` or `msteams.groupPolicy: "open"`.

### [WebChat](https://docs.sage.ai/web/webchat)
...
- [Start with the docs index for navigation and “what’s where.”](https://docs.sage.ai)
- [Read the architecture overview for the gateway + protocol model.](https://docs.sage.ai/concepts/architecture)
- [Use the full configuration reference when you need every key and example.](https://docs.sage.ai/gateway/configuration)
- [Run the Gateway by the book with the operational runbook.](https://docs.sage.ai/gateway)
- [Learn how the Control UI/Web surfaces work and how to expose them safely.](https://docs.sage.ai/web)
- [Understand remote access over SSH tunnels or tailnets.](https://docs.sage.ai/gateway/remote)
- [Follow the onboarding wizard flow for a guided setup.](https://docs.sage.ai/start/wizard)
- [Wire external triggers via the webhook surface.](https://docs.sage.ai/automation/webhook)
- [Set up Gmail Pub/Sub triggers.](https://docs.sage.ai/automation/gmail-pubsub)
- [Learn the macOS menu bar companion details.](https://docs.sage.ai/platforms/mac/menu-bar)
- [Platform guides: Windows (WSL2)](https://docs.sage.ai/platforms/windows), [Linux](https://docs.sage.ai/platforms/linux), [macOS](https://docs.sage.ai/platforms/macos), [iOS](https://docs.sage.ai/platforms/ios), [Android](https://docs.sage.ai/platforms/android)
- [Debug common failures with the troubleshooting guide.](https://docs.sage.ai/channels/troubleshooting)
- [Review security guidance before exposing anything.](https://docs.sage.ai/gateway/security)

## Advanced docs (discovery + control)

- [Discovery + transports](https://docs.sage.ai/gateway/discovery)
- [Bonjour/mDNS](https://docs.sage.ai/gateway/bonjour)
- [Gateway pairing](https://docs.sage.ai/gateway/pairing)
- [Remote gateway README](https://docs.sage.ai/gateway/remote-gateway-readme)
- [Control UI](https://docs.sage.ai/web/control-ui)
- [Dashboard](https://docs.sage.ai/web/dashboard)

## Operations & troubleshooting

- [Health checks](https://docs.sage.ai/gateway/health)
- [Gateway lock](https://docs.sage.ai/gateway/gateway-lock)
- [Background process](https://docs.sage.ai/gateway/background-process)
- [Browser troubleshooting (Linux)](https://docs.sage.ai/tools/browser-linux-troubleshooting)
- [Logging](https://docs.sage.ai/logging)

## Deep dives

- [Agent loop](https://docs.sage.ai/concepts/agent-loop)
- [Presence](https://docs.sage.ai/concepts/presence)
- [TypeBox schemas](https://docs.sage.ai/concepts/typebox)
- [RPC adapters](https://docs.sage.ai/reference/rpc)
- [Queue](https://docs.sage.ai/concepts/queue)

## Workspace & skills

- [Skills config](https://docs.sage.ai/tools/skills-config)
- [Default AGENTS](https://docs.sage.ai/reference/AGENTS.default)
- [Templates: AGENTS](https://docs.sage.ai/reference/templates/AGENTS)
- [Templates: BOOTSTRAP](https://docs.sage.ai/reference/templates/BOOTSTRAP)
- [Templates: IDENTITY](https://docs.sage.ai/reference/templates/IDENTITY)
- [Templates: SOUL](https://docs.sage.ai/reference/templates/SOUL)
- [Templates: TOOLS](https://docs.sage.ai/reference/templates/TOOLS)
- [Templates: USER](https://docs.sage.ai/reference/templates/USER)

## Platform internals

- [macOS dev setup](https://docs.sage.ai/platforms/mac/dev-setup)
- [macOS menu bar](https://docs.sage.ai/platforms/mac/menu-bar)
- [macOS voice wake](https://docs.sage.ai/platforms/mac/voicewake)
- [iOS node](https://docs.sage.ai/platforms/ios)
- [Android node](https://docs.sage.ai/platforms/android)
- [Windows (WSL2)](https://docs.sage.ai/platforms/windows)
- [Linux app](https://docs.sage.ai/platforms/linux)

## Email hooks (Gmail)

- [docs.sage.ai/gmail-pubsub](https://docs.sage.ai/automation/gmail-pubsub)

## Molty

Sage was built for **Molty**, a space lobster AI assistant. 🦞
by Peter Steinberger and the community.

- [sage.ai](https://sage.ai)
- [soul.md](https://soul.md)
- [steipete.me](https://steipete.me)
- [@sage](https://x.com/sage)
