...
# Build and bundle Sage into a minimal .app we can open.
# Outputs to dist/Sage.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Sage.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Sage"
BUNDLE_ID="${BUNDLE_ID:-ai.sage.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/sage/sage/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Sage/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SageBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SageGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Sage"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Sage"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Sage"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Sage" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Sage/Resources/Sage.icns" "$APP_ROOT/Contents/Resources/Sage.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Sage/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SageKit resources"
SAGEKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SageKit_SageKit.bundle"
if [ -d "$SAGEKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SageKit_SageKit.bundle"
  cp -R "$SAGEKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SageKit_SageKit.bundle"
else
  echo "WARN: SageKit resource bundle not found at $SAGEKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Sage"
killall -q Sage 2>/dev/null || true
