...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/sage/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Sage Re-Auth" -c "After getting key, run: ssh $SERVER '~/sage/scripts/mobile-reauth.sh'" --id sage-auth
        ;;
    *)
