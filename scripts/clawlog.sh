...
show_usage() {
    cat << EOF
clawlog - Sage Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Sage logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Sage logs flow through the macOS unified log (subsystem: ai.sage).
