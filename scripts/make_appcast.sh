...
ZIP=${1:?"Usage: $0 Sage-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/sage/sage/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Sage-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/sage/sage/releases/download/v${VERSION}/"}
