#!/bin/bash
# Setup Sage Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'sage-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Sage message: Send warning via Sage itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/sage-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/sage-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/sage-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now sage-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status sage-auth-monitor.timer"
echo ""
echo "Quick commands:"
