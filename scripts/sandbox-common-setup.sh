...
BASE_IMAGE="${BASE_IMAGE:-sage-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-sage-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=sage.sandbox=1)
NOTE
