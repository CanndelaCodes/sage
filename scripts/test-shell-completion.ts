 ...
 * `sage update`. Use it to verify the completion installation flow
...
\`sage update\`, \`sage doctor\`, and \`sage onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Sage Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`sage completion --install\` later to enable.`));
    return;
  }
