...
INSTALL_URL="${SAGE_INSTALL_URL:-https://sage.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SAGE_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SAGE_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="sage"
PACKAGE_NAME="${SAGE_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SAGE_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SAGE_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SAGE_INSTALL_LATEST_OUT:-}"
fi
