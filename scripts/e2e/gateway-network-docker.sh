...
IMAGE_NAME="sage-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="sage-net-e2e-$$"
GW_NAME="sage-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SAGE_GATEWAY_TOKEN=$TOKEN" \
	  -e "SAGE_SKIP_CHANNELS=1" \
	  -e "SAGE_SKIP_GMAIL_WATCHER=1" \
	  -e "SAGE_SKIP_CRON=1" \
	  -e "SAGE_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
