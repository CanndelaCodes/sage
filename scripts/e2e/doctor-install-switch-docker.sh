...
IMAGE_NAME="sage-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/sage-bin:$PATH"
  mkdir -p /tmp/sage-bin

  cat > /tmp/sage-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/sage-bin/systemctl

  cat > /tmp/sage-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_bin="/tmp/npm-prefix/bin/sage"
  npm_entry="/tmp/npm-prefix/lib/node_modules/sage/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/sage.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/sage-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/sage-gateway.service"
    if [ ! -f "$unit_path" ]; then
