...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/sage-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/sage-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/sage-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.sage/workspace"
    config_path="$HOME/.sage/sage.json"
    sessions_dir="$HOME/.sage/agents/main/sessions"
