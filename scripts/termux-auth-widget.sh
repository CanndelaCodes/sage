...
SERVER="${SAGE_SERVER:-${SAGEBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Sage auth..."

STATUS=$(ssh "$SERVER" '$HOME/sage/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/sage/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|SAGE_EXPIRING|SAGEBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/sage/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|SAGE_EXPIRED|SAGEBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Sage won't work until re-auth"
                ;;
        esac
        ;;
