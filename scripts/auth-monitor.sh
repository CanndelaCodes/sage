...
#
# Suggested cron: */30 * * * * /home/admin/sage/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Sage notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., sage-alerts)
...
STATE_FILE="$HOME/.sage/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Sage if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use sage
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Sage to $NOTIFY_PHONE..."
            sage send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Sage is down. Run: ssh l36 '~/sage/scripts/mobile-reauth.sh'" "urgent"
    exit 1
