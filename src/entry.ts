...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.SAGE_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.SAGE_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.SAGE_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[sage] ${parsed.error}`);
    process.exit(2);
  }
