...
import type { SageConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareSageVersions } from "./version.js";
...
function coerceConfig(value: unknown): SageConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SageConfig;
}
...
function stampConfigVersion(cfg: SageConfig): SageConfig {
...
function warnIfConfigFromFuture(cfg: SageConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSageVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Sage (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: SageConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SageConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as SageConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SageConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SageConfig) {
    clearConfigCache();
...
// module scope. `SAGE_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SAGE_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SAGE_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export async function writeConfigFile(cfg: SageConfig): Promise<void> {
  clearConfigCache();
