...
import type { SageConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SageConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: SageConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SageSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SageConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SageConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as SageConfig)),
    ),
  };
}
