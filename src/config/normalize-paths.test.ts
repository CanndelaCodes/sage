  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.sage/logs/sage.log" },
        hooks: {
          path: "~/.sage/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".sage", "logs", "sage.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".sage", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".sage", "telegram.token"),
      );
