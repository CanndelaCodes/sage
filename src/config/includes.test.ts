...
const ETC_SAGE_DIR = path.join(ROOT_DIR, "etc", "sage");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "sage.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSagePath(...parts: string[]) {
  return path.join(ETC_SAGE_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcSagePath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "sage.json"))).toEqual({
      shared: true,
    });
  });
});
