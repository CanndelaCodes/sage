...
function resolveChannelConfig(cfg: SageConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: SageConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: SageConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: SageConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: SageConfig): boolean {
...
function isIMessageConfigured(cfg: SageConfig): boolean {
...
function isWhatsAppConfigured(cfg: SageConfig): boolean {
...
function isGenericChannelConfigured(cfg: SageConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: SageConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SageConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: SageConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: SageConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: SageConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: SageConfig, pluginId: string): SageConfig {
...
function enablePluginEntry(cfg: SageConfig, pluginId: string): SageConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
