...
declare const __SAGE_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __SAGE_VERSION__ === "string" && __SAGE_VERSION__) ||
  process.env.SAGE_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `sage --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.SAGE_GATEWAY_PORT ??
    process.env.SAGEBOT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.SAGE_GATEWAY_BIND ??
    process.env.SAGEBOT_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
    bindRaw === "loopback" ||
