...
async function main() {
  ...
  const { ensureSageCliOnPath } = await import("../infra/path-env.js");
  ensureSageCliOnPath();
  ...
  process.on("uncaughtException", (error) => {
    console.error("[sage] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[sage] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
