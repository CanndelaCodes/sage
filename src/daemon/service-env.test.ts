...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SAGE_GATEWAY_PORT).toBe("18789");
    expect(env.SAGE_GATEWAY_TOKEN).toBe("secret");
    expect(env.SAGE_SERVICE_MARKER).toBe("sage");
    expect(env.SAGE_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SAGE_SERVICE_VERSION).toBe("string");
    expect(env.SAGE_SYSTEMD_UNIT).toBe("sage-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SAGE_LAUNCHD_LABEL).toBe("ai.sage.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SAGE_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SAGE_SYSTEMD_UNIT).toBe("sage-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SAGE_LAUNCHD_LABEL).toBe("ai.sage.work");
    }
  });
});
