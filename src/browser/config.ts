import type { BrowserConfig, BrowserProfileConfig, SageConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_SAGE_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "sage" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SAGE_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SAGE_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Sage browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for sage/sage CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SageConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SAGE_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "sage";
