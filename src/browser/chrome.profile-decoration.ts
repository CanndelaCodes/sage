...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".sage-profile-decorated");
}
 ...
 */
export function decorateSageProfile(userDataDir: string, opts?: { name?: string; color?: string }) {
  const desiredName = opts?.name ?? DEFAULT_SAGE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SAGE_BROWSER_COLOR).toUpperCase();
