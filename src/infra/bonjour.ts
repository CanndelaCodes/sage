...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SAGE_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Sage";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Sage\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.SAGE_MDNS_HOSTNAME?.trim() || process.env.SAGEBOT_MDNS_HOSTNAME?.trim() || "sage";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
