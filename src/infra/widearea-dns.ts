...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SAGE_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*sage-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "sage";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "sage");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "sage-gw");
  ...
  records.push(`_sage-gw._tcp IN PTR ${instanceLabel}._sage-gw._tcp`);
  records.push(`${instanceLabel}._sage-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._sage-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; sage-content-hash: ${contentHash}\n${contentBody}`;
}
