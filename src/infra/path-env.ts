...
function candidateBinDirs(opts: EnsureSagePathOpts): string[] {
  ...
  // Bundled macOS app: `sage` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "sage");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/sage` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "sage"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `sage` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSageCliOnPath(opts: EnsureSagePathOpts = {}) {
  if (isTruthyEnvValue(process.env.SAGE_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.SAGE_PATH_BOOTSTRAPPED = "1";
