...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sage-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "sage doctor",
        managerScriptArgs(manager, "sage", ["doctor", "--non-interactive"]),
        gitRoot,
        { SAGE_UPDATE_IN_PROGRESS: "1" },
      ),
    );
