...
homepage: https://docs.sage.ai/hooks#command-logger
metadata:
  {
    "sage":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.sage/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.sage/logs/commands.log ~/.sage/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/sage`:
   ```
   /home/username/.sage/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
