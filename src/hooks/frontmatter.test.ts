...
`;
    ...
    expect(parsed.sage.emoji).toBe("💾");
    expect(parsed.sage.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.sage.emoji).toBe("📝");
    expect(parsed.sage.events).toEqual(["command"]);
    expect(parsed.sage.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.sage.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"sage": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"sage": {"events": ["test"]}}');
  });
...
describe("resolveSageMetadata", () => {
  it("extracts sage metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSageMetadata(frontmatter);
  ...
  it("returns undefined when sage key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        sage: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Sage" },
            { id: "npm", kind: "npm", package: "@sage/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@sage/hook");
  });
    ...
    "sage":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Sage" }],
      },
  }
---
...
`;
    ...
    const sage = resolveSageMetadata(frontmatter);
    expect(sage).toBeDefined();
    expect(sage?.emoji).toBe("💾");
    expect(sage?.events).toEqual(["command:new"]);
    expect(sage?.requires?.config).toEqual(["workspace.dir"]);
    expect(sage?.install?.[0].kind).toBe("bundled");
  });
