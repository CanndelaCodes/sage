...
import type { SageConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: SageConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: SageConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SAGE_STATE_DIR on a local-only volume and re-run "${formatCliCommand("sage security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: SageConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
...
export function collectHooksHardeningFindings(cfg: SageConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: SageConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: SageConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: SageConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SageConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SageConfig): boolean {
...
function isBrowserEnabled(cfg: SageConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: SageConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectExposureMatrixFindings(cfg: SageConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
