...
${formatHelpExamples([
  ['sage agent --to +15555550123 --message "status update"', "Start a new session."],
  ['sage agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'sage agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'sage agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['sage agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'sage agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.sage.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.sage.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['sage agents set-identity --agent main --name "Sage" --emoji "🦞"', "Set name + emoji."],
  ["sage agents set-identity --agent main --avatar avatars/sage.png", "Set avatar path."],
  [
    "sage agents set-identity --workspace ~/.sage/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "sage agents set-identity --identity-file ~/.sage/workspace/IDENTITY.md --agent main",
