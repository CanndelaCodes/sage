...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    SAGE_STATE_DIR: process.env.SAGE_STATE_DIR,
    SAGE_CONFIG_PATH: process.env.SAGE_CONFIG_PATH,
    SAGE_GATEWAY_PORT: process.env.SAGE_GATEWAY_PORT,
    SAGE_PROFILE: process.env.SAGE_PROFILE,
  };

  beforeEach(() => {
    process.env.SAGE_STATE_DIR = "/tmp/sage-cli-state";
    process.env.SAGE_CONFIG_PATH = "/tmp/sage-cli-state/sage.json";
    delete process.env.SAGE_GATEWAY_PORT;
    delete process.env.SAGE_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.SAGE_STATE_DIR !== undefined) {
      process.env.SAGE_STATE_DIR = originalEnv.SAGE_STATE_DIR;
    } else {
      delete process.env.SAGE_STATE_DIR;
    }

    if (originalEnv.SAGE_CONFIG_PATH !== undefined) {
      process.env.SAGE_CONFIG_PATH = originalEnv.SAGE_CONFIG_PATH;
    } else {
      delete process.env.SAGE_CONFIG_PATH;
    }

    if (originalEnv.SAGE_GATEWAY_PORT !== undefined) {
      process.env.SAGE_GATEWAY_PORT = originalEnv.SAGE_GATEWAY_PORT;
    } else {
      delete process.env.SAGE_GATEWAY_PORT;
    }

    if (originalEnv.SAGE_PROFILE !== undefined) {
      process.env.SAGE_PROFILE = originalEnv.SAGE_PROFILE;
    } else {
      delete process.env.SAGE_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        SAGE_PROFILE: "dev",
        SAGE_STATE_DIR: "/tmp/sage-daemon-state",
        SAGE_CONFIG_PATH: "/tmp/sage-daemon-state/sage.json",
        SAGE_GATEWAY_PORT: "19001",
      },
