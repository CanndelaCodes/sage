...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.sage${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.SAGE_PROFILE = profile;

  const stateDir = env.SAGE_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.SAGE_STATE_DIR?.trim()) {
    env.SAGE_STATE_DIR = stateDir;
  }

  if (!env.SAGE_CONFIG_PATH?.trim()) {
    env.SAGE_CONFIG_PATH = path.join(stateDir, "sage.json");
  }

  if (profile === "dev" && !env.SAGE_GATEWAY_PORT?.trim()) {
    env.SAGE_GATEWAY_PORT = "19001";
  }
}
