...
import { ensureSageCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.SAGE_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.SAGE_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureSageCliOnPath({ pathEnv: process.env.PATH ?? "" });
