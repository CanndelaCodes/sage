...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as SageConfig);
...
export function listChannelAgentTools(params: { cfg?: SageConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: SageConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
