...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.SAGE_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-test-"));
    process.env.SAGE_STATE_DIR = stateDir;
    process.env.SAGE_PROFILE = "isolated";

    try {
      const tool = createSageTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: sage --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/sage" } }\n}\n';
