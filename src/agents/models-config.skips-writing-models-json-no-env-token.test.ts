...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "sage-models-" });
}

const MODELS_CONFIG: SageConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureSageModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureSageModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureSageModelsJson } = await import("./models-config.js");
      const { resolveSageAgentDir } = await import("./agent-paths.js");

      await ensureSageModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveSageAgentDir(), "models.json");
