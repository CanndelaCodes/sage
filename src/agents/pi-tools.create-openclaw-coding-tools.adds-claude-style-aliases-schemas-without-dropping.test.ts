...
import { __testing, createSageCodingTools } from "./pi-tools.js";
import { createSandboxedReadTool } from "./pi-tools.read.js";
import { createSageTools } from "./sage-tools.js";
...
describe("createSageCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSageCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "sage-outside.txt");
