...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): SageConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-workspace-"));
    try {
