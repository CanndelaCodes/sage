...
/**
 * Cleanup suspended Sage CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: SageConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-cli-images-"));
  const paths: string[] = [];
