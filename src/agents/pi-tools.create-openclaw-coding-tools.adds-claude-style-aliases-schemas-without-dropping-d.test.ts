...
const defaultTools = createSageCodingTools();

describe("createSageCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createSageCodingTools();
    ...
    try {
      ...
      const contents = "Hello from sage read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "sage-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "sage-workspace"),
    ...
    const tools = createSageCodingTools({ sandbox });
