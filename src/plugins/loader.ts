...
import { discoverSagePlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SagePluginDefinition;
  register?: SagePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SagePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SagePluginDefinition;
...
export function loadSagePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSagePlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "sage/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SagePluginModule | null = null;
    try {
      mod = jiti(candidate.source) as SagePluginModule;
    } catch (err) {
