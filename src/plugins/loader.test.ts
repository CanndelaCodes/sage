...
import { loadSagePlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.SAGE_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sage-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "sage.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SAGE_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadSagePlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.SAGE_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadSagePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSagePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
