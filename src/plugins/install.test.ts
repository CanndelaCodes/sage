...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sage-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.sage/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without sage.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@sage/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("sage.extensions");
  });
});
