...
export function listAgentEntries(cfg: SageConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SageConfig, agentId: string) {
...
function resolveAgentModel(cfg: SageConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SageConfig): AgentSummary[] {
