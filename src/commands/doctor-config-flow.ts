...
function stripUnknownConfigKeys(config: SageConfig): {
  config: SageConfig;
  removed: string[];
} {
  const parsed = SageSchema.safeParse(config);
  if (parsed.success) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".sage");
  const targetPath = path.join(targetDir, "sage.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("sage doctor --fix")}" to apply these changes.`);
    }
  }
