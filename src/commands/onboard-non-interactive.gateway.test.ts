...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.SAGE_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sage-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.SAGE_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "sage");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.SAGE_STATE_DIR = stateDir;
    process.env.SAGE_CONFIG_PATH = path.join(stateDir, "sage.json");
