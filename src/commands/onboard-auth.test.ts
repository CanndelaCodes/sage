...
const requireAgentDir = () => {
  const agentDir = process.env.SAGE_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SAGE_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  const previousAgentDir = process.env.SAGE_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.SAGE_AGENT_DIR;
    } else {
      process.env.SAGE_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.SAGE_OAUTH_DIR;
  });

  it("writes auth-profiles.json under SAGE_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-oauth-"));
    process.env.SAGE_STATE_DIR = tempStateDir;
    process.env.SAGE_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SAGE_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SAGE_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-minimax-"));
    process.env.SAGE_STATE_DIR = tempStateDir;
    process.env.SAGE_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SAGE_AGENT_DIR;
