...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Sage status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("sage security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("sage security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.sage.ai/faq");
  runtime.log("Troubleshooting: https://docs.sage.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("sage status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("sage logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("sage status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("sage gateway probe")}`);
  }
}
