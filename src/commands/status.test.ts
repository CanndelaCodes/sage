...
beforeAll(() => {
  previousProfile = process.env.SAGE_PROFILE;
  process.env.SAGE_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.SAGE_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/sage-root.js", () => ({
  resolveSagePackageRoot: vi.fn().mockResolvedValue("/tmp/sage"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/sage/pnpm-lock.yaml",
      markerPath: "/tmp/sage/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Sage status"))).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SAGE_GATEWAY_TOKEN = "abcd1234";
    try {
