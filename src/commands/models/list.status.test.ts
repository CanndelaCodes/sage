...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSageAgentDir: vi.fn().mockReturnValue("/tmp/sage-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/sage-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/sage-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSageAgentDir: mocks.resolveSageAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSageAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/sage-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/sage-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/sage-agent-custom");
