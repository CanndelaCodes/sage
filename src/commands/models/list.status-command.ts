...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSageAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("sage models auth setup-token")}\` or \`${formatCliCommand("sage configure")}\`.`
          : `Run \`${formatCliCommand("sage configure")}\` or set an API key env var.`;
