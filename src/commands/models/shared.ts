...
import { type SageConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function updateConfig(mutator: (cfg: SageConfig) => SageConfig): Promise<SageConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: SageConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SageConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("sage agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
