...
import { readConfigFileSnapshot, type SageConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: SageConfig, model: string): SageConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("sage plugins install")}\`.`,
    );
  }
