...
import type { SageConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: SageConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("sage security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("sage configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("sage config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("sage doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("sage config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
