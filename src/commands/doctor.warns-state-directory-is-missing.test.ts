...
beforeEach(() => {
  ...
  resolveSagePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SAGE_STATE_DIR;
  originalUpdateInProgress = process.env.SAGE_UPDATE_IN_PROGRESS;
  process.env.SAGE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sage-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SAGE_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SAGE_UPDATE_IN_PROGRESS;
  } else {
    process.env.SAGE_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveSagePackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadSagePlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/sage-root.js", () => ({
  resolveSagePackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "sage-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SAGE_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SAGE_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/sage.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SAGE_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
