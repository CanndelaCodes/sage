...
const SEARCH_TOOL = "https://docs.sage.ai/mcp.SearchSage";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.sage.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('sage docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.sage.ai/");
      runtime.log(`Search: ${formatCliCommand('sage docs "your query"')}`);
    }
    return;
  }
