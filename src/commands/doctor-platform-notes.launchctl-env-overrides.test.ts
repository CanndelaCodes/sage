...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SAGE_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SAGE_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SAGE_GATEWAY_TOKEN");
    expect(message).not.toContain("SAGE_GATEWAY_PASSWORD");
  });
