...
function setSlackDmPolicy(cfg: SageConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Sage";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Sage`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: SageConfig, allowFrom: string[]): SageConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
