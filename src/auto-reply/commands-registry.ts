...
export function isCommandEnabled(cfg: SageConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SageConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as SageConfig),
...
export function getCommandDetection(_cfg?: SageConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SageConfig) {
