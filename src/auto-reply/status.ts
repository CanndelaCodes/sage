...
type AgentConfig = Partial<NonNullable<NonNullable<SageConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.sage/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
const formatVoiceModeLine = (config?: SageConfig, sessionEntry?: SessionEntry): string | null => {
  if (!config) {
    return null;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Sage ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: SageConfig): string {
  const lines = ["ℹ️ Help", ""];
