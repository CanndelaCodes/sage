...
import { scheduleGatewaySigusr1Restart, triggerSageRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Sage in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSageRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Sage via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
