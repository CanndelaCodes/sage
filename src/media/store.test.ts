...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "SAGE_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "sage-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.SAGE_STATE_DIR = path.join(home, ".sage");
    ...
    await fs.mkdir(path.join(home, ".sage"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.sage${path.sep}media`);
