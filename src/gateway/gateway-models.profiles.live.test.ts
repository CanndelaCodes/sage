...
import type { SageConfig, ModelProviderConfig } from "../config/types.js";
import { resolveSageAgentDir } from "../agents/agent-paths.js";
...
import { ensureSageModelsJson } from "../agents/models-config.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.SAGE_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SAGE_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SAGE_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SAGE_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<SageConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.SAGE_CONFIG_PATH,
    token: process.env.SAGE_GATEWAY_TOKEN,
    skipChannels: process.env.SAGE_SKIP_CHANNELS,
    skipGmail: process.env.SAGE_SKIP_GMAIL_WATCHER,
    skipCron: process.env.SAGE_SKIP_CRON,
    skipCanvas: process.env.SAGE_SKIP_CANVAS_HOST,
    agentDir: process.env.SAGE_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.SAGE_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.SAGE_SKIP_CHANNELS = "1";
  process.env.SAGE_SKIP_GMAIL_WATCHER = "1";
  process.env.SAGE_SKIP_CRON = "1";
  process.env.SAGE_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.SAGE_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveSageAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-live-state-"));
  process.env.SAGE_STATE_DIR = tempStateDir;
  ...
  process.env.SAGE_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.sage-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSageAgentDir();
  const sanitizedCfg: SageConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-live-"));
  const tempConfigPath = path.join(tempDir, "sage.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.SAGE_CONFIG_PATH = tempConfigPath;

  await ensureSageModelsJson(nextCfg);
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Sage live tool probe (local, safe): " +
    ...
    process.env.SAGE_CONFIG_PATH = previous.configPath;
    process.env.SAGE_GATEWAY_TOKEN = previous.token;
    process.env.SAGE_SKIP_CHANNELS = previous.skipChannels;
    process.env.SAGE_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.SAGE_SKIP_CRON = previous.skipCron;
    process.env.SAGE_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.SAGE_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SAGE_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureSageModelsJson(cfg);
      ...
      const rawModels = process.env.SAGE_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.sage-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
