...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SAGE_SKIP_CHANNELS;
    prevSkipGmail = process.env.SAGE_SKIP_GMAIL_WATCHER;
    process.env.SAGE_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SAGE_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SAGE_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SAGE_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
