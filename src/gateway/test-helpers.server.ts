...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.SAGE_STATE_DIR;
  previousConfigPath = process.env.SAGE_CONFIG_PATH;
  previousSkipBrowserControl = process.env.SAGE_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.SAGE_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.SAGE_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sage-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SAGE_STATE_DIR = path.join(tempHome, ".sage");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "sage-test-"))
    : path.join(tempHome, ".sage-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.SAGE_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.SAGE_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.SAGE_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.SAGE_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SAGE_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.SAGE_SKIP_CANVAS_HOST;
    } else {
      process.env.SAGE_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.SAGE_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SAGE_GATEWAY_TOKEN = fallbackToken;
  }
