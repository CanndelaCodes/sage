...
const originalConnect = SageApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("sage-app") as SageApp;
...
beforeEach(() => {
  SageApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SAGE_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  SageApp.prototype.connect = originalConnect;
  window.__SAGE_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/sage/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/sage");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/sage/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SAGE_CONTROL_UI_BASE_PATH__ = "/sage";
    const app = mountApp("/sage/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/sage");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/sage/sessions");
  });
