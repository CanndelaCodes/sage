...
metadata: { "sage": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${SAGE_TMUX_SOCKET_DIR:-${SAGEBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/sage-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/sage.sock"
SESSION=sage-python
...
- Use `SAGE_TMUX_SOCKET_DIR` (legacy `SAGEBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$SAGE_TMUX_SOCKET_DIR/sage.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `SAGE_TMUX_SOCKET_DIR`).
