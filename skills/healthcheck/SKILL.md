---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Sage deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Sage cron scheduling for periodic checks, or version status checks on a machine running Sage (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Sage, then align it to a user-defined risk tolerance without breaking access. Use Sage security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Sage changes the host firewall, SSH, or OS updates; it does not.
...
5. Sage gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Sage can access sensitive data.
...
### 2) Run Sage security audits (read-only)

As part of the default read-only checks, run `sage security audit --deep`. Only offer alternatives if the user requests them:

1. `sage security audit` (faster, non-probing)
2. `sage security audit --json` (structured output)

Offer to apply Sage safe defaults (numbered):

1. `sage security audit --fix`

Be explicit that `--fix` only tightens Sage defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Sage version/update status (read-only)

As part of the default read-only checks, run `sage update status`.
...
- Credential hygiene notes (location of Sage creds, prefer disk encryption)
...
After Sage install or first hardening pass, run at least one baseline audit and version check:

- `sage security audit`
- `sage security audit --deep`
- `sage update status`

Ongoing monitoring is recommended. Use the Sage cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `sage cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `sage update status`
...
Before creating, `sage cron list` and match on exact `name`. If found, `sage cron edit <id> ...`.
If not found, `sage cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `sage update status` (preferred for source checkouts and channels)
2. `npm view sage version` (published npm version)

## Sage command accuracy

Use only supported commands and flags:

- `sage security audit [--deep] [--fix] [--json]`
- `sage status` / `sage status --deep`
- `sage health --json`
- `sage update status`
- `sage cron add|list|runs|run`

Do not invent CLI flags or imply Sage enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Sage compaction:
