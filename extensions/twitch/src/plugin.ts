/**
 * Twitch channel plugin for Sage.
 *
...
import { buildChannelConfigSchema } from "sage/plugin-sdk";
import type {
 ...
 * for Sage. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SageConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SageConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SageConfig): boolean => {
