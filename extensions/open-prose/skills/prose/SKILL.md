...
metadata: { "sage": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Sage `sessions_spawn`
- **File I/O** == Sage `read`/`write`
- **Remote fetch** == Sage `web_fetch` (or `exec` with curl when POST is required)
