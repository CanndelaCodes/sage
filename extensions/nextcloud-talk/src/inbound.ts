...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as SageConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as SageConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as SageConfig);
