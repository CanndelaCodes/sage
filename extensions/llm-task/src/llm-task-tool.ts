...
// NOTE: This extension is intended to be bundled with Sage.
// When running from source (tests/dev), Sage internals live under src/.
...
export function createLlmTaskTool(api: SagePluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via sage.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sage-llm-task-"));
