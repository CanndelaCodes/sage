...
`sage.invoke --each`) without adding new Sage code per workflow.
...
This extension depends on Sage internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Sage extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.sage/extensions` as a standalone plugin directory.
