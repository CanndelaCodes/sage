...
Sage extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
sage channels login --channel zalouser
...
```bash
sage message send --channel zalouser --target <threadId> --message "Hello from Sage!"
```
...
```bash
sage channels login --channel zalouser              # Login via QR
sage channels login --channel zalouser --account work
sage channels status --probe
sage channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
sage directory self --channel zalouser
sage directory peers list --channel zalouser --query "name"
sage directory groups list --channel zalouser --query "work"
sage directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
sage message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
sage message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `sage channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
sage channels login --channel zalouser --account work
sage message send --channel zalouser --account work --target <id> --message "Hello"
