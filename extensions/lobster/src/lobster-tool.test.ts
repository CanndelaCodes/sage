...
import type { SagePluginApi, SagePluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "sage-lobster-plugin-") {
...
function fakeApi(overrides: Partial<SagePluginApi> = {}): SagePluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<SagePluginToolContext> = {}): SagePluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: SagePluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
