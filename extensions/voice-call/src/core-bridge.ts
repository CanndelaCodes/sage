...
function resolveSageRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SAGE_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set SAGE_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSageRoot(), "dist", "extensionAPI.js");
