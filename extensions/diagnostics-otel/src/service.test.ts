...
vi.mock("sage/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("sage/plugin-sdk")>("sage/plugin-sdk");
  return {
    ...actual,
...
import { emitDiagnosticEvent } from "sage/plugin-sdk";
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(spanNames).toContain("sage.webhook.processed");
    expect(spanNames).toContain("sage.message.processed");
    expect(spanNames).toContain("sage.session.stuck");
