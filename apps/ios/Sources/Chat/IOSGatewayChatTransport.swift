...
struct IOSGatewayChatTransport: SageChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SageChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SageChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SageChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SageChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SageChatAttachmentPayload]) async throws -> SageChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SageChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(SageChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(SageGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SageGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SageChatEventPayload.self)
                        {
