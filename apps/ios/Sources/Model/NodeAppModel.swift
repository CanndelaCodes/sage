...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = SageCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = SageCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = SageCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = SageCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = SageCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__sage__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: SageLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for sage:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case SageLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case SageCanvasCommand.present.rawValue,
                 SageCanvasCommand.hide.rawValue,
                 SageCanvasCommand.navigate.rawValue,
                 SageCanvasCommand.evalJS.rawValue,
                 SageCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case SageCanvasA2UICommand.reset.rawValue,
                 SageCanvasA2UICommand.push.rawValue,
                 SageCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case SageCameraCommand.list.rawValue,
                 SageCameraCommand.snap.rawValue,
                 SageCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case SageScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SageNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SageNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(SageLocationGetParams.self, from: req.paramsJSON)) ??
            SageLocationGetParams()
        ...
        let payload = SageLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SageCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(SageCanvasPresentParams.self, from: req.paramsJSON)) ??
                SageCanvasPresentParams()
        ...
        case SageCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SageCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(SageCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SageCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(SageCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SageCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(SageCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SageCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing sageA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SageCanvasA2UICommand.push.rawValue, SageCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == SageCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(SageCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try SageCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(SageCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try SageCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.sageA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SageCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SageCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(SageCameraSnapParams.self, from: req.paramsJSON)) ??
                SageCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SageCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(SageCameraClipParams.self, from: req.paramsJSON)) ??
                SageCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SageScreenRecordParams.self, from: req.paramsJSON)) ??
            SageScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> SageLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return SageLocationMode(rawValue: raw) ?? .off
    }
