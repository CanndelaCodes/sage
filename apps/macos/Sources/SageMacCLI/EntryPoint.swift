...
struct SageMacCLI {
    static func main() async {
        ...
        default:
            fputs("sage-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      sage-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      sage-mac discover [--timeout <ms>] [--json] [--include-local]
      sage-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      sage-mac connect
      sage-mac connect --url ws://127.0.0.1:18789 --json
      sage-mac discover --timeout 3000 --json
      sage-mac wizard --mode local
    """)
}
