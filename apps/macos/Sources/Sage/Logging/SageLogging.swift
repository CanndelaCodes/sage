...
enum SageLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = SageOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SageFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SageLogging.bootstrapIfNeeded()
        let label = SageLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SageOSLogHandler: LogHandler {
...
struct SageFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SageLogging.parseLabel(self.label)
