...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SAGE_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/sage")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("sage-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("sage-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/sage/ (rolling or stdout), or nil if none exist.
