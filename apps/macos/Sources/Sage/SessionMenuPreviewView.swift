...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.sage", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SageSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SageSessionPreviewEntry,
    ...
    private static func cache(payload: SageSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SageChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SageKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SageKit.AnyCodable]) -> [SageChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SageChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SageChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SageChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SageChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SageChatMessage) -> String? {
