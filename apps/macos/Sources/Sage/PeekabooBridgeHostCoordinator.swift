...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.sage", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SagePeekabooBridgeServices?
    private static var sageSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Sage", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = SagePeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.sageSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.sageSocketPath, privacy: .public)")
    }
...
@MainActor
private final class SagePeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.sage.peekaboo")
