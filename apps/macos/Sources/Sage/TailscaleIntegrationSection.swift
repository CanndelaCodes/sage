...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.sage/sage.json. Prefer SAGE_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.sage/sage.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.sage/sage.json. Restart the gateway to apply."
        }
