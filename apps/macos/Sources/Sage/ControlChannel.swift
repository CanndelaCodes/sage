...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SageProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.sage", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SageKit.AnyCodable]()) {
                $0[$1.key] = SageKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SageProtocol.AnyCodable?) -> [String: SageProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SageProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SageKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SageProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("sage.control.heartbeat")
    static let controlAgentEvent = Notification.Name("sage.control.agent")
}
