...
private let webChatSwiftLogger = Logger(subsystem: "ai.sage", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SageChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SageChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SageChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SageChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SageGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SageChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SageChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SageChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SageChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SageChatView>) -> NSViewController
    {
