...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.sage", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.sage", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [SageChatMessage] = messages.compactMap { item in
