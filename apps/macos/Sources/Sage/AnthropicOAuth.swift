...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Sage token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SageOAuthStore.AnthropicOAuthStatus = SageOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.sage", category: "anthropic-oauth")
...
enum SageOAuthStore {
    ...
    private static let sageOAuthDirEnv = "SAGE_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Sage OAuth token file not found"
            case .unreadableFile: "Sage OAuth token file not readable"
            case .invalidJSON: "Sage OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Sage OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Sage OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.sageOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".sage", isDirectory: true)
