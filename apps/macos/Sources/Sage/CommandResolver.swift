...
enum CommandResolver {
    private static let projectRootDefaultsKey = "sage.gatewayProjectRootPath"
    private static let helperName = "sage"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let sageEntry = root.appendingPathComponent("sage.mjs").path
        if FileManager().isReadableFile(atPath: sageEntry) { return sageEntry }
        let binEntry = root.appendingPathComponent("bin/sage.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/sage")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let sagePaths = self.sageManagedPaths(home: home)
        if !sagePaths.isEmpty {
            extras.insert(contentsOf: sagePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + sagePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func sageManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".sage"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func sageExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSageExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("sage.mjs").path,
            root.appendingPathComponent("bin/sage.js").path,
        ]
    ...
    static func hasAnySageInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.sageExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let sagePath = self.projectSageExecutable(projectRoot: root) {
                return [sagePath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "sage", subcommand] + extraArgs
            }
            if let sagePath = self.sageExecutable(searchPaths: searchPaths) {
                return [sagePath, subcommand] + extraArgs
            }

            let missingEntry = """
            sage entrypoint missing (looked for dist/index.js or sage.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real sage CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/sage"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/sage.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/sage.mjs"
            node "$PRJ/sage.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/sage.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/sage.js"
            node "$PRJ/bin/sage.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent sage"
          pnpm --silent sage \(quotedArgs);
        else
          echo "sage CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SageConfigFile.loadDict()
