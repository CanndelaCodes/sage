...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SageOAuthStore.AnthropicOAuthStatus = SageOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: SageOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SageOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SageOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SageOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Sage can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SageOAuthStore.AnthropicOAuthStatus,
