...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SageProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SageProtocol.AnyCodable] = [
            "presence": .init([SageProtocol.AnyCodable(entry)]),
        ]
        let payload = SageProtocol.AnyCodable(payloadMap)
