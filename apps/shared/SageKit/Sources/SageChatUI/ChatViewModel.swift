...
private let chatUILogger = Logger(subsystem: "ai.sage", category: "SageChatUI")

@MainActor
@Observable
public final class SageChatViewModel {
    public private(set) var messages: [SageChatMessage] = []
    ...
    public var attachments: [SagePendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SageChatPendingToolCall] = []
    public private(set) var sessions: [SageChatSessionEntry] = []
    private let transport: any SageChatTransport
    ...
    private var pendingToolCallsById: [String: SageChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SageChatTransport) {
    ...
    public var sessionChoices: [SageChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [SageChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [SageChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SagePendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SageChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SageChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [SageChatMessage]) -> [SageChatMessage] {
        var result: [SageChatMessage] = []
    ...
    private static func dedupeKey(for message: SageChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SageChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> SageChatAttachmentPayload in
            SageChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SageChatSessionEntry {
        SageChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SageChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SageChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SageAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SageChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SagePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
