...
private final class TestChatTransport: @unchecked Sendable, SageChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SageChatHistoryPayload]
    private let sessionsResponses: [SageChatSessionsListResponse]

    private let stream: AsyncStream<SageChatTransportEvent>
    private let continuation: AsyncStream<SageChatTransportEvent>.Continuation

    init(
        historyResponses: [SageChatHistoryPayload],
        sessionsResponses: [SageChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SageChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SageChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SageChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [SageChatAttachmentPayload]) async throws -> SageChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return SageChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SageChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SageChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SageChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SageChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SageChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SageChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SageChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SageChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SageChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SageChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
