...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Sage
...
Running Sage via Node with `tsx` fails at startup with:

```
[sage] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `sage status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch sage.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node sage.mjs status` works on Node 25.
