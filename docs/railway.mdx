...
Deploy Sage on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/sagebot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/sage` — Control UI

## What you get

- Hosted Sage Gateway + Control UI
...
- `SAGE_STATE_DIR=/data/.sage` (recommended)
- `SAGE_WORKSPACE_DIR=/data/workspace` (recommended)
- `SAGE_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Sage state + workspace so you can migrate to another host without losing config or memory.
