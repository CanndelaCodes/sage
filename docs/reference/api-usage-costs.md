...
Sage features that can generate provider usage or paid API calls.
...
- `sage status --usage` and `sage channels list` show provider **usage windows**
...
Sage can pick up credentials from:
...
`sage models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
