...
Sage integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Sage owns lifecycle when `channels.signal.autoStart=true`.
...
- Sage spawns `imsg rpc` as a child process (legacy iMessage integration).
