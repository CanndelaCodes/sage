...
You can also set `SAGE_VERBOSE=1` instead of the flag.
...
### `docs.sage.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.sage.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.sage.ai` to the allowlist, then retry.
...
- Check the allowlist: `sage config get agents.defaults.models`
...
If you can, include the relevant log tail from `sage logs --follow`.
