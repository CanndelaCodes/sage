---
summary: "Sage on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Sage on a Raspberry Pi
  - Running Sage on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Sage on Raspberry Pi

## Goal

Run a persistent, always-on Sage Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Sage features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled sage
...
```bash
# Check logs
journalctl -u sage --no-pager -n 100

# Common fix: rebuild
cd ~/sage  # if using hackable install
npm run build
sudo systemctl restart sage
```
