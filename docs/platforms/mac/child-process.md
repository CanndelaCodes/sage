...
service via the external `sage` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`SAGE_PROFILE`; legacy `com.sage.*` is supported).
