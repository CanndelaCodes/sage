...
read_when:
  - "You installed Sage but `sage` is “command not found”"
...
Sage’s runtime baseline is **Node 22+**.

If you can run `npm install -g sage@latest` but later see `sage: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
