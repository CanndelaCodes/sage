...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/sage`)
...
If the page fails to load, start the Gateway first: `sage gateway`.
...
you revoke it with `sage devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.sage/sage.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Sage
...
Sage **blocks** Control UI connections without device identity.
