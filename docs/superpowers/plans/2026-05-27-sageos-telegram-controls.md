...
- Modify: `src/telegram/bot-native-commands.test.ts`

- [x] **Step 1: Write the failing Telegram test**
...
Run: `pnpm exec vitest run --config vitest.unit.config.ts src/telegram/bot-native-commands.test.ts -t SageOS`
Expected: FAIL until the registry/handler path is complete. In this repo state, the first run exposed a test harness mock gap in the existing skill-command mock; production dispatch was already wired.

- [x] **Step 3: Verify native dispatch**
...
Run:
`pnpm exec vitest run --config vitest.unit.config.ts src/auto-reply/commands-registry.test.ts src/auto-reply/reply/commands.test.ts src/telegram/bot-native-commands.test.ts`

- [x] **Step 2: Run static checks**

Run:
`pnpm oxfmt --check docs/superpowers/plans/2026-05-27-sageos-telegram-controls.md src/auto-reply/commands-registry.data.ts src/auto-reply/commands-registry.test.ts src/auto-reply/reply/commands-sageos.ts src/auto-reply/reply/commands-core.ts src/auto-reply/reply/commands.test.ts src/telegram/bot-native-commands.test.ts`
`git diff --check`
`pnpm tsgo`
`pnpm lint`
`pnpm build`
