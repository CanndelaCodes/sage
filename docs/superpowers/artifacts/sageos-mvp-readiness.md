...
| Overlay-native Sage AI Chat                                                                                                                          | Green  | Overlay renderer model, `system/chat` workspace, Gateway chat RPC helpers, controller chat event state, visual contract anti-collapse guard, packaged Electron smoke, visual review packet, OpenAI ChatKit/Agents SDK boundary decision                                                                                                                                                                                                | Re-run overlay chat and smoke gates     |
| Hermes Agent and Hermes Desktop replacement benchmark                                                                                                | Yellow | Core MVP spec now defines the replacement requirement against the current Hermes Desktop pattern: same-agent continuity, daily desktop UI, shared sessions/history, live tools, file/artifact preview, voice/settings/skills/cron/profiles/messaging/agent management, memory, approvals, and audit evidence. Current SageOS surfaces cover many of these areas but still need a final parity audit against Jason's daily Hermes loop. | Complete Hermes parity audit with Jason |
...
| Enterprise-grade Liquid Linear overlay visual gate                                                                                                   | Yellow | Visual contract tests, no-live-texture/transparent-blur guard, see-through material alpha guard, explicit glass legibility treatments, runtime reduced-motion smoke, app-like underlay screenshots, screenshot pixel-content sampling, Liquid Linear spec, smoke artifact, and `review:visual:all` pass                                                                                                                                | Jason visual acceptance                 |
...
- The Windows overlay now has overlay-native Sage AI Chat backed by the local Gateway chat protocol, including recent `chat.history` transcript rendering, visible composer controls, streaming state, session reset, and a Command Deck `system/chat` drill-down.
