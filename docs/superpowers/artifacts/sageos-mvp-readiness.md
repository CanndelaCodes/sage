...
- Overlay-native Sage AI Chat is a first-class full-mode workspace backed by the local Gateway chat protocol.
- The MVP chat UI is custom overlay UI; OpenAI ChatKit and the OpenAI Agents SDK are reference points or future adapters, not replacements for the overlay shell or Gateway contract.
...
- SageOS must be capable of replacing Hermes Agent and Hermes Desktop as Jason's primary AI agent; Hermes parity gaps that keep Hermes required for the main daily loop are MVP blockers unless Jason explicitly scopes them out.
...
- High-level MVP spec: `docs/superpowers/specs/2026-05-18-sageos-command-center-design.md`, including the overlay-native Sage AI Chat and OpenAI SDK boundary decision.
- Overlay visual direction: `docs/superpowers/specs/2026-06-01-sageos-liquid-linear-overlay-design.md`.
- Hermes Desktop parity audit: `docs/superpowers/artifacts/sageos-hermes-desktop-parity-audit.md`.
...
| Configurable global hotkey opens/closes overlay                                                                                                      | Green  | Electron adapter, window controller, production launch script, startup shortcut helper, smoke evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Re-run overlay smoke                     |
| Game Bar-style native shell layer                                                                                                                    | Green  | Electron adapter now creates a frameless transparent full-display overlay window, explicitly reapplies display bounds, `skipTaskbar`, fullscreenable capability, and `screen-saver` always-on-top level for full, HUD, and Edge Rail surfaces. Packaged Electron smoke now asserts the window is display-bounds-sized and topmost before continuing.                                                                                                                                                                                                                     | Jason live Windows acceptance            |
...
| Overlay-native Sage AI Chat                                                                                                                          | Green  | Overlay renderer model, `system/chat` workspace, Gateway chat RPC helpers, recent `chat.history`, recent `sessions.list`, overlay session resume, controller chat event state, visual contract anti-collapse guard, packaged Electron smoke, visual review packet, OpenAI ChatKit/Agents SDK boundary decision                                                                                                                                                                                                                                                           | Re-run overlay chat and smoke gates      |
| Hermes Agent and Hermes Desktop replacement benchmark                                                                                                | Yellow | Core MVP spec and the Hermes Desktop parity audit now define the replacement requirement against the audited Hermes source pattern: same-agent continuity, native desktop UI, shared sessions/history, streaming tools, command palette, file/artifact preview, voice, provider/model/tool/MCP settings, skills/cron/profile-equivalent controls, agent/activity management, memory, approvals, and audit evidence. Current SageOS now covers overlay-native chat history/session continuity and live overlay control, but still needs Jason's daily-loop parity review. | Complete Hermes parity review with Jason |
...
| Visual review companion for Jason acceptance                                                                                                         | Green  | `pnpm --dir apps/windows-overlay review:visual:all` runs Electron smoke capture, regenerates the local acceptance packet, verifies 11 loaded screenshots, 8 criteria, 8 rubric rows, 24 decision controls, the acceptance decision recorder, no broken images, 0 pixel-content failures, and no horizontal overflow; `review:visual:serve` exposes the packet on localhost and can save a durable local acceptance artifact                                                                                                                                              | Jason visual acceptance                  |
...
| Enterprise-grade Liquid Linear overlay visual gate                                                                                                   | Yellow | Visual contract tests, transparent renderer root guard, no-full-screen-blur/sheet guard, no-live-texture guard, see-through material alpha guard, Liquid Linear ripple token, explicit glass legibility treatments, runtime reduced-motion smoke, app-like underlay screenshots, screenshot pixel-content sampling, Liquid Linear spec, smoke artifact, and `review:visual:all` pass                                                                                                                                                                                     | Jason visual acceptance                  |
...
- Visual review companion closeout on 2026-06-04:
  ...
  - `pnpm --dir apps/windows-overlay review:visual:verify` launched `C:\Program Files\Google\Chrome\Application\chrome.exe`, loaded the generated file, verified 11 loaded screenshots, 8 acceptance criteria, 0 broken images, 0 horizontal overflow, and saved `apps/windows-overlay/dist/overlay-visual-review-render.png`.
...
- Liquid Linear transparency and pixel-content closeout on 2026-06-04:
  - `pnpm --dir apps/windows-overlay exec vitest run tests/overlay-visual-contract.test.ts tests/smoke-script.test.ts tests/visual-review-script.test.ts --reporter verbose` passed with 3 files and 16 tests after the overlay root stopped applying full-screen blur, the smoke fixtures moved to app-like desktop/sheet/terminal/browser/IDE underlays, and the visual verifier gained local-file canvas sampling.
  - `pnpm --dir apps/windows-overlay review:visual:all` passed with `buildSha: 01aecb784212`, 11 screenshots, 8 criteria, 8 rubric rows, 24 decision controls, 3 release decision cards, 0 broken images, 0 horizontal overflow, and `imageContentFailures: []`.
  - `pnpm --dir apps/windows-overlay test` passed with 14 files and 95 tests, `pnpm --dir apps/windows-overlay typecheck` passed, and `pnpm --dir apps/windows-overlay build` passed.
  - Codex visually inspected `apps/windows-overlay/dist/overlay-smoke-styled.png`, `overlay-smoke-edge-browser.png`, `overlay-smoke-edge-ide.png`, and `overlay-smoke-hud.png`; the refreshed screenshots no longer show a dot-matrix/full-screen texture and preserve the underlying desktop/app context.
- Full visual review gate closeout on 2026-06-04:
  ...
  - `pnpm --dir apps/windows-overlay review:visual:all` ran `smoke:electron`, `review:visual`, and `review:visual:verify` in order, launched `C:\Program Files\Google\Chrome\Application\chrome.exe`, loaded the generated review sheet, verified 11 loaded screenshots, 8 acceptance criteria, 8 reviewer rubric rows, 24 acceptance decision controls, 3 release decision cards, 0 broken images, 0 horizontal overflow, saved `apps/windows-overlay/dist/overlay-visual-review-render.png`, and wrote `apps/windows-overlay/dist/overlay-visual-review-report.json` with `humanAcceptance: "required"`.
  - A follow-up acceptance-recorder pass added local decision persistence, a visible release-state summary, and copyable JSON in the review page. `review:visual:verify` now selects all criteria as accepted and confirms the recorder emits `humanAcceptance: "accepted"` with release state `All criteria accepted`, then flips one criterion to `mvp-blocker` and confirms release state `MVP blocked`.
  - A localhost serving pass added `review:visual:serve`, which serves the generated review packet from `apps/windows-overlay/dist` on `127.0.0.1` with dist-only path protection. The packet was opened in the Codex in-app browser at `http://127.0.0.1:58231/` and verified with 24 decision controls, 11 screenshots, and neutral state `Human decision incomplete`.
...
- Production overlay launch closeout on 2026-06-04:
  - `pnpm --dir apps/windows-overlay exec vitest run tests/smoke-script.test.ts --reporter verbose` first failed because the package had no built-app `start` script and the PowerShell launch/startup scripts did not expose production build-mode control.
  - The overlay package now has `pnpm --dir apps/windows-overlay start`, the launch script defaults to `-Build auto`, rebuilds missing assets, supports `-Build always`, fails fast with `-Build never` when required assets are missing, and starts the built Electron app instead of the development script.
  - The current-user startup shortcut helper preserves the selected `-Build auto|always|never` mode.
- Overlay-native Sage AI Chat closeout on 2026-06-04:
  - `pnpm --dir apps/windows-overlay exec vitest run tests/overlay-renderer.test.ts --reporter verbose` first failed because the overlay model had no `Sage AI Chat` panel, no Command Deck `system/chat` resource, and no `system/chat` workspace.
  - `pnpm --dir apps/windows-overlay exec vitest run tests/sageos-actions.test.ts tests/overlay-controller.test.ts --reporter verbose` first failed because the overlay lacked `chat.history`, `chat.send`, and `chat.abort` helpers, controller send/abort methods, and Gateway `chat` event state.
  - The full overlay now renders a first-class Sage AI Chat panel with session, transport, delivery, Gateway session-state facts, transcript area, composer, Send, Stop, and New session controls. It sends through `chat.send` with `deliver: false`, aborts active runs through `chat.abort`, and tracks streamed/final/aborted/error `chat` events.
  - `pnpm --dir apps/windows-overlay smoke:electron` first caught an accessible-name collision between the global SageOS `Stop` button and the chat `Stop` button, then passed after chat controls received disambiguating ARIA labels.
  - A one-off Electron bounding-box inspection caught the chat panel body overflowing under the Command Deck; the visual contract now enforces `.sage-ai-chat` content rows and `min-height: 336px`, and the packaged Electron smoke now asserts visible chat facts, transcript, composer, controls, minimum usable heights, and no Command Deck overlap at runtime.
  - `pnpm --dir apps/windows-overlay test` passed with 14 files and 101 tests, `pnpm --dir apps/windows-overlay typecheck` passed, `pnpm --dir apps/windows-overlay build` passed, `pnpm --dir apps/windows-overlay smoke:electron` passed with `rendererErrors: 0`, and `pnpm --dir apps/windows-overlay review:visual:all` passed with 11 screenshots and 0 pixel-content failures.
- OpenAI chat SDK direction on 2026-06-04:
  - Current official OpenAI docs position ChatKit as an embeddable agentic chat UI and the Agents SDK as a code-owned orchestration layer for tools, handoffs, tracing, guardrails, MCP integrations, and custom state.
  - SageOS MVP keeps Sage AI Chat as a custom overlay-native UI backed by the local Sage Gateway chat protocol, because the chat surface must integrate with Command Deck, Agent Workspace, HUD, Edge Rail, pass-through behavior, approvals, incidents, audit, and Sage Memory boundaries.
  - ChatKit is a post-MVP reference or supplemental web Command Center candidate for attachments, inline widgets, actions, starter prompts, theming, and richer thread ergonomics. The Agents SDK is a post-MVP backend adapter candidate that must sit behind Sage's existing Gateway/session/run contracts if adopted.
- Sage AI Chat history follow-up on 2026-06-04:
  - Focused TDD first failed because `loadChatHistory` stored raw Gateway messages and the packaged Electron smoke did not exercise `chat.history`.
  - The overlay controller now normalizes `chat.history` payloads into displayable transcript rows with stable keys, role labels, and text extraction for string, top-level `text`, and OpenAI-style content-array messages.
  - The renderer now loads recent history on Gateway hello and renders prior user, Sage, system, tool, and unknown messages before the current live draft/stream.
  - The packaged Electron smoke now exposes `chat.history` in the mock Gateway, returns a visible history fixture, asserts the historical user and assistant messages render in the overlay-native Sage AI Chat panel, anchors the pass-through probe to the overlay display, and forces the current test BrowserWindow into pass-through before native underlay click verification.
  - `pnpm --dir apps/windows-overlay exec vitest run tests/overlay-controller.test.ts tests/smoke-script.test.ts --reporter verbose` passed with 20 tests, `pnpm --dir apps/windows-overlay smoke:electron` passed with `chat.history` calls and `rendererErrors: 0`, and `pnpm --dir apps/windows-overlay review:visual:all` passed with 11 screenshots and 0 pixel-content failures.
- Sage AI Chat session continuity follow-up on 2026-06-04:
  - Focused TDD first failed because the overlay had no `sessions.list` RPC helper, no recent-session model, no Sessions control, and no overlay-native way to resume a selected Gateway session.
  - The overlay controller now loads recent Gateway sessions through `sessions.list`, normalizes session titles, previews, and updated timestamps for the chat panel, and resumes a selected session by reloading `chat.history` for that session key.
  - The renderer now loads recent sessions on Gateway hello, renders a Recent sessions strip, marks the active session, exposes a mouse-first Sessions refresh control, and preserves the session list when starting a new overlay session.
  - The packaged Electron smoke now advertises `sessions.list`, returns a visible recent-session fixture, clicks `Resume Sage AI session telegram`, asserts the selected session key and resumed transcript render, and includes the session strip/buttons in runtime fit and layout checks.
- Transparent root and liquid ripple follow-up on 2026-06-04:
  - The overlay renderer root now uses `background: transparent` and `backdrop-filter: none`, keeping the desktop or open Windows apps visible behind the overlay instead of drawing a full-screen solid or blurred page sheet.
  - The Electron adapter now restores focusability for full command mode, removes focus for HUD/Edge Rail and pass-through mode, and blurs before pass-through so native underlay clicks reach the target app.
  - The Liquid Linear material system now includes `--sageos-liquid-ripple` plus pressed ripple feedback for buttons, overlay buttons, cards, panels, Agent Workspace, pinned widgets, overlay rows, and recent chat sessions.
  - Focused visual-contract and smoke tests now require the transparent root, no full-screen renderer blur, ripple token/selectors, and `@keyframes sageos-liquid-ripple`.
- Game Bar-style shell overlay follow-up on 2026-06-04:
  - The native Electron shell now treats the overlay as a full-display Windows layer rather than a normal work-area app window: the BrowserWindow is frameless, transparent, taskbar-skipping, menu-hidden, display-bounds-sized, and reasserted at the `screen-saver` always-on-top level for full overlay, HUD, and Edge Rail surfaces.
  - Packaged Electron smoke now asserts the overlay window is topmost and exactly matches the active display bounds before running interaction checks. This guards against regressing to a normal desktop app window that leaves the taskbar above or appears as a taskbar application.
  - The pass-through probe was hardened so the underlay probe stays above ordinary app windows while the overlay remains the topmost layer, keeping the native click-through check stable on multi-monitor Windows setups.
- Hermes Desktop parity source audit on 2026-06-04:
  - A source audit of `https://github.com/NousResearch/hermes-agent` at commit `d1367355d514b5ce3af6056ca660ab28e9d632e4` recorded Hermes Desktop's Electron shell, same-agent runtime bootstrap, chat, sessions, command palette, file/preview surfaces, voice, provider/model/tool/MCP settings, subagent/activity views, and scoped approval-bypass pattern.
  - The new artifact `docs/superpowers/artifacts/sageos-hermes-desktop-parity-audit.md` translates those findings into SageOS MVP parity targets and keeps the benchmark Yellow until Jason can run his normal Hermes daily loop in SageOS or approves named post-MVP exceptions.
- Visual acceptance artifact save closeout on 2026-06-04:
  - `pnpm --dir apps/windows-overlay exec vitest run tests/visual-review-script.test.ts --reporter verbose` first failed because the visual review page had no save control and `review:visual:serve` had no `POST /acceptance` endpoint, then passed after the page gained `Save acceptance JSON` and the localhost server gained validation plus atomic writes to `apps/windows-overlay/dist/overlay-visual-review-acceptance.json`.
  - HTTP checks against `http://127.0.0.1:58232/` verified the root page returned `200`, the save button rendered, invalid acceptance packets returned `400`, a valid packet returned `{ ok: true, file: "overlay-visual-review-acceptance.json" }`, and the saved JSON was retrievable with `humanAcceptance: "accepted"`.
  - A Playwright/Chrome browser check selected all 8 Accept radio controls, clicked `Save acceptance JSON`, observed `Saved overlay-visual-review-acceptance.json`, and read back a saved artifact with 8 decisions. This proves the save mechanics only; it does not replace Jason's human visual/product acceptance.
...
- The Windows overlay now has a production runtime launch path: build assets are checked before startup, missing assets are built by default, and startup shortcuts can preserve an explicit build mode.
- The Windows overlay now has overlay-native Sage AI Chat backed by the local Gateway chat protocol, including recent `chat.history` transcript rendering, recent `sessions.list` session discovery, overlay session resume, visible composer controls, streaming state, session reset, and a Command Deck `system/chat` drill-down.
- The Windows overlay renderer root is now truly transparent, so the desktop/open apps underneath remain visible while the panels, menus, rows, widgets, and chat sessions provide the glass material and subtle pressed ripple feedback.
- The native Windows shell now behaves more like a Game Bar-style overlay layer: no ordinary taskbar presence is requested, the window covers the full display bounds rather than only work area, and the `screen-saver` topmost level is reasserted whenever the overlay, HUD, or Edge Rail is shown.
- The MVP chat direction is explicitly custom and Gateway-backed; OpenAI ChatKit and Agents SDK are recorded as post-MVP references or adapters rather than replacement UI/runtime substrates.
...
- Overlay visual smoke now also covers desktop-like app underlays and rejects blank or flat screenshots through browser canvas pixel sampling.
...
- A local visual review companion now turns the smoke screenshot set into one Jason acceptance packet with release decision cards, a reviewer rubric, local decision persistence, copyable acceptance JSON, a localhost serving command, a durable verification report, and a one-command gate that runs smoke capture, acceptance-packet generation, and browser verification.
- The localhost visual review companion can now durably save a reviewer acceptance packet to `apps/windows-overlay/dist/overlay-visual-review-acceptance.json`, giving Jason's final acceptance a concrete local artifact once he records it.
- Public Windows docs now match the overlay-first MVP status instead of describing Windows desktop support as absent.
- Liquid Linear command glass is tokenized and test-covered in the overlay visual contract, including no live full-screen texture, transparent material alpha ceilings, explicit blur/rim/shadow legibility treatments, HUD, Edge Rail, ambient pinned widgets, and foreground stacking above specular material layers.
- Hermes Desktop parity is now source-audited against the cloned NousResearch repo and tracked as an MVP benchmark artifact rather than a vague competitor note.
...
- Jason should live-test that the production overlay has no taskbar button and appears above the Windows taskbar on his monitor layout; automated smoke verifies the Electron side but cannot inspect the real taskbar icon row directly.
- The overlay is production-usable as an MVP candidate, but the final release should still include Jason's visual pass against the current visual review acceptance packet and representative Windows desktop backgrounds.
- Hermes Agent and Hermes Desktop replacement remains a named MVP benchmark until Jason can run his normal daily Hermes loop in SageOS without keeping Hermes as the primary agent.
- Hermes-like advanced preview rail, file browsing, provider/model/tool/MCP setup, advanced session search/archive, and voice parity remain Yellow until Jason confirms they are not required to stop using Hermes Desktop as the primary daily agent UI.
- Sage AI Chat is MVP-usable and overlay-native, but richer ChatKit-style thread features such as attachments, inline widgets, action cards, and advanced history search remain post-MVP.
