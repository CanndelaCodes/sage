...
- A local visual review companion now turns the smoke screenshot set into one Jason acceptance packet with release decision cards, a reviewer rubric, local decision persistence, copyable acceptance JSON, a durable verification report, and a one-command gate that runs smoke capture, acceptance-packet generation, and browser verification.
