...
- Visual review companion closeout on 2026-06-04:
  - `pnpm --dir apps/windows-overlay exec vitest run tests/visual-review-script.test.ts --reporter verbose` first failed because the package script and generator did not exist, then passed with 1 file and 2 tests after `review:visual` and `scripts/build-visual-review.mjs` were added.
