...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `sage sandbox explain`.
...
Legacy `agent.*` configs are migrated by `sage doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=sage-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${SAGE_STATE_DIR:-$HOME/.sage}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
