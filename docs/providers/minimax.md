---
summary: "Use MiniMax M2.1 in Sage"
read_when:
  - You want MiniMax models in Sage
...
```bash
sage plugins enable minimax-portal-auth  # skip if already loaded.
sage gateway restart  # restart if gateway is already running
sage onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/sage/sage/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `sage.json`:
...
- Use `sage models list` and `sage models set minimax/MiniMax-M2.1` to switch.
...
- Running `sage configure` and selecting **MiniMax M2.1**, or
