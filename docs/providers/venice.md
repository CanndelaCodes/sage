---
summary: "Use Venice AI privacy-focused models in Sage"
read_when:
  - You want privacy-focused inference in Sage
...
```bash
sage onboard --auth-choice venice-api-key
```
...
```bash
sage chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Sage shows all available Venice models. Pick based on your needs:
...
```bash
sage models set venice/claude-opus-45
sage models set venice/llama-3.3-70b
```

List all available models:

```bash
sage models list | grep venice
```

## Configure via `sage configure`

1. Run `sage configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Sage automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
sage chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
sage chat --model venice/claude-opus-45

# Use uncensored model
sage chat --model venice/venice-uncensored

# Use vision model with image
sage chat --model venice/qwen3-vl-235b-a22b

# Use coding model
sage chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `sage models list` to see currently available models. Some models may be temporarily offline.
