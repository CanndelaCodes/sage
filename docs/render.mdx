...
Deploy Sage on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/sage/sage"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run Sage:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SAGE_STATE_DIR
        value: /data/.sage
      - key: SAGE_WORKSPACE_DIR
        value: /data/workspace
      - key: SAGE_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/sage`.
...
If you use the original Sage repository, Render will not auto-deploy your Sage. To update it, run a manual Blueprint sync from the dashboard.
...
For Sage, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Sage host.
