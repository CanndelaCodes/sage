...
read_when:
  - You want Sage to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Sage to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`sage onboard`) and `sage channels add` list optional channel plugins.
...
```bash
sage plugins install @sage/nostr
```

Use a local checkout (dev workflows):

```bash
sage plugins install --link <path-to-sage>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "sage@example.com",
        "lud16": "sage@example.com"
      }
    }
  }
}
