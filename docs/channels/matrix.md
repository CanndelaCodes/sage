...
Matrix is an open, decentralized messaging protocol. Sage connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `sage plugins install @sage/matrix`
   - From a local checkout: `sage plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Sage calls the same
     login endpoint, stores the access token in `~/.sage/credentials/matrix/credentials.json`,
...
- On first connection, Sage requests device verification from your other sessions.
...
`~/.sage/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Sage resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
