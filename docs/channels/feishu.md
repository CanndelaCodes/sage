...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Sage to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed Sage, run the wizard:
...
2. Configuring app credentials in Sage
...
1. You already ran `sage channels add` for Feishu
2. The gateway is running (`sage gateway status`)
...
2. Run `sage logs --follow` and look for `chat_id`
...
2. Run `sage logs --follow` and look for `open_id`
...
| `sage gateway restart` | Restart gateway service       |
| `sage logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `sage gateway status`
6. Check logs: `sage logs --follow`
