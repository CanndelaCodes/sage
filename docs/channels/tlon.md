...
Tlon is a decentralized messenger built on Urbit. Sage connects to your Urbit ship and can
...
```bash
sage plugins install @sage/tlon
```

Local checkout (when running from a git repo):

```bash
sage plugins install ./extensions/tlon
```
...
Use these with `sage message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Sage replies in-thread.
