...
```bash
sage plugins install @sage/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
sage plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Sage" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `sage pairing list nextcloud-talk`
  - `sage pairing approve nextcloud-talk <CODE>`
