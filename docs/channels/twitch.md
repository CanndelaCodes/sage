...
read_when:
  - Setting up Twitch chat integration for Sage
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Sage connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
sage plugins install @sage/twitch
```

Local checkout (when running from a git repo):

```bash
sage plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SAGE_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "sage", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SAGE_TWITCH_ACCESS_TOKEN env var)
...
```bash
SAGE_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
