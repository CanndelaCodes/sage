...
3. Set tokens for Sage and start the gateway.
...
7. Slash Commands → create `/sage` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Sage can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Sage falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Sage",
    "description": "Slack connector for Sage"
  },
  "features": {
    "bot_user": {
      "display_name": "Sage",
    ...
    "slash_commands": [
      {
        "command": "/sage",
        "description": "Send a message to Sage",
        "should_escape": false
      }
    ]
  },
...
By default, Sage replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Sage infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `sage pairing approve slack <code>`.
...
- On startup, Sage resolves channel/user names in allowlists to IDs (when tokens allow)
