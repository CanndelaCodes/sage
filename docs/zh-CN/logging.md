...
所有日志配置都在 `~/.sage/sage.json` 的 `logging` 下。
...
- Sage 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `sage plugins enable diagnostics-otel` 启用插件。
...
- `sage.tokens`（计数器，属性：`sage.token`、`sage.channel`、`sage.provider`、`sage.model`）
- `sage.cost.usd`（计数器，属性：`sage.channel`、`sage.provider`、`sage.model`）
- `sage.run.duration_ms`（直方图，属性：`sage.channel`、`sage.provider`、`sage.model`）
- `sage.context.tokens`（直方图，属性：`sage.context`、`sage.channel`、`sage.provider`、`sage.model`）

消息流：

- `sage.webhook.received`（计数器，属性：`sage.channel`、`sage.webhook`）
- `sage.webhook.error`（计数器，属性：`sage.channel`、`sage.webhook`）
- `sage.webhook.duration_ms`（直方图，属性：`sage.channel`、`sage.webhook`）
- `sage.message.queued`（计数器，属性：`sage.channel`、`sage.source`）
- `sage.message.processed`（计数器，属性：`sage.channel`、`sage.outcome`）
- `sage.message.duration_ms`（直方图，属性：`sage.channel`、`sage.outcome`）

队列 + 会话：

- `sage.queue.lane.enqueue`（计数器，属性：`sage.lane`）
- `sage.queue.lane.dequeue`（计数器，属性：`sage.lane`）
- `sage.queue.depth`（直方图，属性：`sage.lane` 或 `sage.channel=heartbeat`）
- `sage.queue.wait_ms`（直方图，属性：`sage.lane`）
- `sage.session.state`（计数器，属性：`sage.state`、`sage.reason`）
- `sage.session.stuck`（计数器，属性：`sage.state`）
- `sage.session.stuck_age_ms`（直方图，属性：`sage.state`）
- `sage.run.attempt`（计数器，属性：`sage.attempt`）
...
- `sage.webhook.error`
  - `sage.channel`、`sage.webhook`、`sage.chatId`、`sage.error`
- `sage.message.processed`
  - `sage.channel`、`sage.outcome`、`sage.chatId`、`sage.messageId`、`sage.sessionKey`、`sage.sessionId`、`sage.reason`
- `sage.session.stuck`
  - `sage.state`、`sage.ageMs`、`sage.queueDepth`、`sage.sessionKey`、`sage.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `sage doctor`。
