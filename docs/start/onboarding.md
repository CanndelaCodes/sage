---
summary: "First-run onboarding flow for Sage (macOS app)"
read_when:
...
- Writes credentials to `~/.sage/credentials/oauth.json`
...
The app can install the global `sage` CLI via npm/pnpm so terminal
...
On the first agent run, Sage bootstraps a workspace (default `~/.sage/workspace`):
...
```bash
sage webhooks gmail setup --account you@gmail.com
```
