...
```bash
sage tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `sage tui --deliver`
...
- Check the Gateway logs: `sage logs --follow`.
- Confirm the agent can run: `sage status` and `sage models status`.
...
- No agents in picker: check `sage agents list` and your routing config.
