...
- Publish as `sage/plugin-sdk` (or export from core under `sage/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SagePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SageConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SageConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SageConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SageConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SageConfig): string;
  };
};
```
...
- Introduce `sage/plugin-sdk`.
- Add `api.runtime` to `SagePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `sageRuntime: ">=2026.2.0"`).
