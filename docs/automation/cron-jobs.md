...
- Jobs persist under `~/.sage/cron/` so restarts don’t lose schedules.
...
sage cron list
sage cron run <job-id> --force
sage cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.sage/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `sage cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Sage defaults to `announce`.
...
- Job store: `~/.sage/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.sage/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
sage cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
sage cron edit <jobId> --agent ops
sage cron edit <jobId> --clear-agent
```
...
```bash
sage system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`sage system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SAGE_SKIP_CRON`.
