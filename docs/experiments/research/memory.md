...
read_when:
  - Designing workspace memory (~/.sage/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Sage integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.sage/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.sage/workspace` as canonical human-readable memory.
...
```
~/.sage/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Sage**, but keep a separable core library.

### Why integrate into Sage?
...
- You want the agent itself to call the tools:
  - `sage memory recall "…" --k 25 --since 30d`
  - `sage memory reflect --since 7d`
...
Pragmatic take for `~/.sage/workspace`:
