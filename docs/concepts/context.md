...
read_when:
  - You want to understand what “context” means in Sage
...
“Context” is **everything Sage sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Sage-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Sage builds the system prompt

The system prompt is **Sage-owned** and rebuilt each run. It includes:
...
By default, Sage injects a fixed set of workspace files (if present):
