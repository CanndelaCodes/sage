...
- `/usage cost` in chats: local cost summary aggregated from Sage session logs.
- CLI: `sage status --usage` prints a full per-provider breakdown.
- CLI: `sage channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
