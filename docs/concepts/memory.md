---
summary: "How Sage memory works (workspace files + automatic memory flush)"
read_when:
...
When a session is **close to auto-compaction**, Sage triggers a **silent,
...
Sage can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Uses remote embeddings by default. If `memorySearch.provider` is not set, Sage auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. Sage
...
BM25 + vectors + reranking. Markdown stays the source of truth; Sage shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.sage/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.sage/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `qmd query --json`. If QMD fails or the binary is missing,
  Sage automatically falls back to the builtin SQLite manager so memory tools
  keep working.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - Sage sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index Sage
    uses), run a one-off query with the agent’s XDG dirs.

    Sage’s QMD state lives under your **state dir** (defaults to `~/.sage`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    Sage uses:

    ```bash
    # Pick the same state dir Sage uses
    STATE_DIR="${SAGE_STATE_DIR:-$HOME/.sage}"
    if [ -d "$HOME/.moltbot" ] && [ ! -d "$HOME/.sage" ] \
      && [ -z "${SAGE_STATE_DIR:-}" ]; then
...
- When `memory.qmd.sessions.enabled = true`, Sage exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.sage/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
When enabled, Sage combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Sage falls back to vector-only search.
...
Sage can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, Sage stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Sage logs the
