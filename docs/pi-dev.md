...
This guide summarizes a sane workflow for working on the pi integration in Sage.
...
- Trigger the agent directly:
  - `pnpm sage agent --message "Hello" --thinking low`
...
State lives under the Sage state directory. Default is `~/.sage`. If `SAGE_STATE_DIR` is set, use that directory instead.

To reset everything:

- `sage.json` for config
...
- https://docs.sage.ai/testing
- https://docs.sage.ai/start/getting-started
