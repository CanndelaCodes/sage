---
summary: "Chrome extension: let Sage drive your existing Chrome tab"
read_when:
...
The Sage Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate sage-managed Chrome profile.
...
Sage then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Sage release (npm package) as static files. There is no separate “build” step.

After upgrading Sage:

- Re-run `sage browser extension install` to refresh the installed files under your Sage state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Sage ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Sage to control.
...
- `ON`: attached; Sage can drive that tab.
...
Debugging: `sage sandbox explain`
...
`sage browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `sage browser extension install` first to copy the extension to a stable location under your Sage state directory.
...
- **This is not isolated** like the dedicated sage-managed profile.
