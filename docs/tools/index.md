---
summary: "Agent tool surface for Sage (browser, canvas, nodes, message, cron) replacing legacy `sage-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `sage-*` skills
title: "Tools"
---

# Tools (Sage)

Sage exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `sage-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `sage.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Sage logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`sage node run`).
...
- Requires a Brave API key (recommended: `sage configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Sage-managed browser.
...
- Quick smoke: `sage nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `sage gateway` restart in-place)
