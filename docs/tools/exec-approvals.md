...
- **gateway host** → `sage` process on the gateway machine
...
`~/.sage/exec-approvals.json` directly.

CLI: `sage approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
