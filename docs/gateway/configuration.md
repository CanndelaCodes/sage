---
summary: "All configuration options for ~/.sage/sage.json with examples"
read_when:
...
Sage reads an optional **JSON5** config from `~/.sage/sage.json` (comments + trailing commas allowed).

If the file is missing, Sage uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.sage/workspace`). You usually only need a config to:
...
Sage only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `sage doctor`, `sage logs`, `sage health`, `sage status`, `sage service`, `sage help`).
- Run `sage doctor` to see the exact issues.
- Run `sage doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `sage config set`. Keep a backup of `~/.sage/sage.json`.
...
```bash
sage gateway call config.get --params '{}' # capture payload.hash
sage gateway call config.apply --params '{
...
```bash
sage gateway call config.get --params '{}' # capture payload.hash
sage gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.sage/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@sage", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.sage/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.sage/workspace" }],
}
```
...
{ "$include": "/etc/sage/base.json5" } // absolute
...
```json5
// ~/.sage/clients/mueller/agents.json5
[
...
```json5
// ~/.sage/clients/mueller/broadcast.json5
{
...
Sage reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Sage runs your login shell and imports only the missing expected keys (never overrides).
...
- `SAGE_LOAD_SHELL_ENV=1`
- `SAGE_SHELL_ENV_TIMEOUT_MS=15000`
...
Sage stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.sage/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.sage/credentials/oauth.json` (or `$SAGE_STATE_DIR/credentials/oauth.json`)
...
- `~/.sage/agent/*` (migrated by `sage doctor` into `~/.sage/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `SAGE_OAUTH_DIR`
- Agent dir (default agent root override): `SAGE_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Sage imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Sage derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/sage/sage-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/sage/sage.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.sage/credentials/whatsapp/biz
          // authDir: "~/.sage/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `sage doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@sage", "sage"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@sage"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.sage/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.sage/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.sage/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.sage/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.sage/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.sage/workspace-home" },
      { id: "work", workspace: "~/.sage/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `sage.json`).
...
Sage starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["sage-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-sage",
...
Sage starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `sage plugins install @sage/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Sage starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Sage spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Sage
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/sage" } },
}
```
...
When a file exceeds this limit, Sage logs a warning and injects a truncated
...
message envelopes). If unset, Sage uses the host timezone at runtime.
...
`[sage]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[sage]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[sage]"`, Sage will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Sage generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.sage/settings/tts.json",
      elevenlabs: {
...
Sage also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Sage currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `sage configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:sage`: all built-in Sage tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.sage/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.sage/sandboxes",
        docker: {
          image: "sage-sandbox:bookworm-slim",
          containerPrefix: "sage-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "sage-sandbox-browser:bookworm-slim",
          containerPrefix: "sage-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Sage uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.sage/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Sage config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Sage writes/merges a `models.json` into
`~/.sage/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Sage uses
...
- Shortcut: `sage onboard --auth-choice opencode-zen`.
...
Shortcut: `sage onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `sage onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `sage onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `sage onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `sage onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `sage onboard --auth-choice minimax-api`.
...
- Override the agent config root with `SAGE_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.sage/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.sage/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.sage/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Sage stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.sage/skills` (workspace skills
...
from `~/.sage/extensions`, `<workspace>/.sage/extensions`, plus any
...
### `browser` (sage-managed browser)

Sage can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for sage and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Sage.app menubar, or `sage gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      sage: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/sage" }
...
- When a connection comes from one of these IPs, Sage uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `sage gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `SAGE_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `SAGE_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Sage
...
- Sage.app watches `~/.sage/sage.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.sage/sage.json` (or `SAGE_CONFIG_PATH`) and applies changes automatically.
...
- `~/.sage/sage.json` (or `SAGE_CONFIG_PATH`)
...
- `SAGE_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `sage --dev …` → uses `~/.sage-dev` + shifts ports from base `19001`
- `sage --profile <name> …` → uses `~/.sage-<name>` (port via config/env/flags)
...
Gmail helper config (used by `sage webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "sage@gmail.com",
...
- Set `SAGE_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Sage defaults `serve.path` to `/` so
...
Default root: `~/.sage/workspace/canvas`  
Default port: `18793` (chosen to avoid the sage browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__sage__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__sage__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.sage/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.sage/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.sage/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_sage-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `sage` (advertises `sage.local`). Override with `SAGE_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_sage-gw._tcp` under `~/.sage/dns/` using the configured discovery domain (example: `sage.internal.`).
