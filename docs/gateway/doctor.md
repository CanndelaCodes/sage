...
`sage doctor` is the repair + migration tool for Sage. It fixes stale
...
- Extra workspace dir detection (`~/sage`).
...
you to run `sage doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.sage/sage.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.sage/sessions/` to `~/.sage/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.sage/agent/` to `~/.sage/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.sage/credentials/*.json` (except `oauth.json`)
  - to `~/.sage/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `sage doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.sage` folders exist across
  home directories or when `SAGE_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.sage/sage.json` is
...
offers to remove them and install the Sage service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Sage gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `sage doctor --generate-gateway-token` to force token
...
- `sage doctor` prompts before rewriting supervisor config.
- `sage doctor --yes` accepts the default repair prompts.
- `sage doctor --repair` applies recommended fixes without prompts.
- `sage doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `sage gateway install --force`.
