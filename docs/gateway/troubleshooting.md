---
summary: "Quick troubleshooting guide for common Sage failures"
read_when:
...
When Sage misbehaves, here's how to fix it.
...
| `sage gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `sage channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `sage gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `sage logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `sage status --all` (it redacts tokens). If you paste `sage status`, consider setting `SAGE_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  sage models auth setup-token --provider anthropic
  ```
...
```bash
sage models auth paste-token --provider anthropic
sage models status
```
...
- Preferred: `sage logs --follow`
- File logs (always): `/tmp/sage/sage-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$SAGE_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u sage-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  sage config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  sage config set gateway.mode remote
  sage config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `sage setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.sage/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `sage doctor`
...
- then run `sage sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `sage gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `sage gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `sage gateway status` and `sage doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `SAGE_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `sage gateway status` shows a config mismatch**
...
- Fix: rerun `sage gateway install --force` from the same `--profile` / `SAGE_STATE_DIR` you want the service to use.

**If `sage gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `sage doctor` to update it (or `sage gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `SAGE_GATEWAY_TOKEN`) and restart the service.

**If `sage gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/sage` on disk.
...
Symptoms: `pwd` or file tools show `~/.sage/sandboxes/...` even though you
...
Sage intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `sage models list` or
  `sage models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${SAGE_CONFIG_PATH:-$HOME/.sage/sage.json}"
```
...
tail -f "$(ls -t /tmp/sage/sage-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
sage logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.sage/agents/<agentId>/sessions/
```
...
sage logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
sage gateway --verbose
```

If you’re logged out / unlinked:

```bash
sage channels logout
trash "${SAGE_STATE_DIR:-$HOME/.sage}/credentials" # if logout can't cleanly remove everything
sage channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/sage/sage-*.log | head -1)" | tail -20
```

### High Memory Usage

Sage keeps conversation history in memory.
...
Sage now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `sage doctor` reports every invalid entry.
- `sage doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `sage logs`, `sage health`, `sage status`, `sage gateway status`, and `sage gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/sage/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `sage models status` (CLI).
...
5. Run `sage channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Sage scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/sage/sage/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
sage gateway status
sage gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.sage.* still works)
```
...
Ensure the global `sage` CLI is installed and matches the app version:

```bash
sage --version
npm install -g sage@<version>
```
...
#   ${SAGE_CONFIG_PATH:-$HOME/.sage/sage.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
sage gateway --verbose
sage channels login --verbose
```
...
trash "${SAGE_STATE_DIR:-$HOME/.sage}"
sage channels login         # re-pair WhatsApp
sage gateway restart           # or: sage gateway
```
...
1. Check logs first: `/tmp/sage/` (default: `sage-YYYY-MM-DD.log`, or your configured `logging.file`)
