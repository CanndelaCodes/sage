...
- During heartbeat runs, Sage treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `sage sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Sage skips the heartbeat run entirely (no model call).
...
```bash
sage system event --text "Check for urgent follow-ups" --mode now
```
