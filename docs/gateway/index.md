...
- Replaces the legacy `gateway` command. CLI entry point: `sage gateway`.
...
- Config hot reload watches `~/.sage/sage.json` (or `SAGE_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__sage__/canvas/` from `~/.sage/workspace/canvas`. Disable with `canvasHost.enabled=false` or `SAGE_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `SAGE_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `SAGE_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.sage.*` may still exist)
- Linux: `sage-gateway-<profile>.service`
- Windows: `Sage Gateway (<profile>)`

Install metadata is embedded in the service config:

- `SAGE_SERVICE_MARKER=sage`
- `SAGE_SERVICE_KIND=gateway`
- `SAGE_SERVICE_VERSION=<version>`
...
```bash
sage --dev setup
sage --dev gateway --allow-unconfigured
...
- `SAGE_STATE_DIR=~/.sage-dev`
- `SAGE_CONFIG_PATH=~/.sage-dev/sage.json`
- `SAGE_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.sage/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `SAGE_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `SAGE_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `SAGE_CONFIG_PATH`
- unique `SAGE_STATE_DIR`
...
```bash
sage --profile main gateway install
sage --profile rescue gateway install
```

Example:

```bash
SAGE_CONFIG_PATH=~/.sage/a.json SAGE_STATE_DIR=~/.sage-a sage gateway --port 19001
SAGE_CONFIG_PATH=~/.sage/b.json SAGE_STATE_DIR=~/.sage-b sage gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `sage health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `sage`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `sage gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.sage.*` is cleaned up).
  - `sage doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Sage profile services.
  ...
  - Cleanup: `sage gateway uninstall` (current service) and `sage doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `sage gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Sage.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.sage.*` labels still unload cleanly).
- To stop it cleanly, use `sage gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `sage gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `sage gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Sage installs a **systemd user service** by default on Linux/WSL2. We
...
`sage gateway install` writes the user unit. `sage doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/sage-gateway[-<profile>].service`:

```
[Unit]
Description=Sage Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/sage gateway --port 18789
Restart=always
RestartSec=5
Environment=SAGE_GATEWAY_TOKEN=
...
```
systemctl --user enable --now sage-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/sage-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now sage-gateway[-<profile>].service
```
...
- `sage gateway health|status` — request health/status over the Gateway WS.
- `sage message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `sage agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `sage gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `sage gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `sage gateway` and the legacy TCP control port.
