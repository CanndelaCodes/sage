...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SAGE_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SAGE_STATE_DIR = path.join(base, ".sage");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "sage-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".sage", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
