...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `sage-e2e-${name}-`));
  const configDir = path.join(homeDir, ".sage");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SAGE_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        SAGE_GATEWAY_TOKEN: inst.gatewayToken,
        SAGE_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SAGE_GATEWAY_PORT: String(gwA.port),
          SAGE_GATEWAY_TOKEN: gwA.gatewayToken,
          SAGE_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SAGE_GATEWAY_PORT: String(gwB.port),
          SAGE_GATEWAY_TOKEN: gwB.gatewayToken,
          SAGE_GATEWAY_PASSWORD: "",
        }),
